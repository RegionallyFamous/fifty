...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'cog', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'cog'          => array(
				'label'       => __( 'Cog', 'cog' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'cog' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'cog' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'cog' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'cog' ),
				'description' => __( 'Full-width hero and banner patterns.', 'cog' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'cog' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'cog' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'cog' ),
				'description' => __( 'Social proof and customer quote patterns.', 'cog' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'cog' ),
				'description' => __( 'Footer layout patterns.', 'cog' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'cog' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Cog voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'cog' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'cog' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'cog' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'cog' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'cog' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'cog' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'cog' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Cog voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum 565d',
			'Proceed to Checkout'                                                           => 'To the register c822',
			'Proceed to checkout'                                                           => 'To the register 1e21',
			'Lost your password?'                                                           => 'Key misplaced 9492',
			'Username or email address'                                                     => 'Cog register 37d3',
			'Username or Email Address'                                                     => 'Cog register be77',
			'+ Add apartment, suite, etc.'                                                  => 'Cog register 0a71',
			'You are currently checking out as a guest.'                                    => 'Cog register e0e2',
			'Showing the single result'                                                     => 'Cog register c691',
			'Default sorting'                                                               => 'Counter choice 07f1',
			'No products were found matching your selection.'                               => 'Shop-floor find a9c5',
			'No products in the cart.'                                                      => 'Parcel basket 11ba',
			'Your cart is currently empty!'                                                 => 'Parcel basket 0e6c',
			'Your cart is currently empty.'                                                 => 'Parcel basket 9084',
			'Return to shop'                                                                => 'Cog register b7a3',
			'Return To Shop'                                                                => 'Cog register 79d5',
			'Have a coupon?'                                                                => 'Voucher slip abcc',
			'Update cart'                                                                   => 'Parcel basket ea5f',
			'Place order'                                                                   => 'To the register bded',
			'Apply coupon'                                                                  => 'Voucher slip 39bb',
			'Coupon code'                                                                   => 'Voucher slip b53c',
			'Order details'                                                                 => 'Parcel record e694',
			'Order summary'                                                                 => 'Parcel record 49a0',
			'Cart subtotal'                                                                 => 'Parcel basket 4473',
			'Add to cart'                                                                   => 'Parcel basket 7b53',
			'Customer details'                                                              => 'Cog register 349f',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Cog register 47a4',
			'Be the first to review'                                                        => 'Counter note eff6',
			'Your review'                                                                   => 'Counter note f3d2',
			'Your rating'                                                                   => 'Cog register f8fd',
			'Submit'                                                                        => 'Cog register 2a62',
			'Description'                                                                   => 'Cog register 00f5',
			'Reviews'                                                                       => 'Counter note 1100',
			'Additional information'                                                        => 'Cog register c8c0',
			'View cart'                                                                     => 'Parcel basket 8f7d',
			'View Cart'                                                                     => 'Parcel basket 8ce7',
			'Choose an option'                                                              => 'Cog register 49cb',
			'Clear'                                                                         => 'Cog register 38c4',
			'Login'                                                                         => 'Cog register c0c5',
			'Log in'                                                                        => 'Cog register 2872',
			'Log out'                                                                       => 'Cog register 94d3',
			'Register'                                                                      => 'Cog register 010d',
			'Remember me'                                                                   => 'Cog register ad71',
			'My account'                                                                    => 'Cog register 9962',
			'My Account'                                                                    => 'Cog register a4a7',
			'Order received'                                                                => 'Parcel record 05d8',
			'Thank you. Your order has been received.'                                      => 'Parcel record 76fa',
			'You may also like&hellip;'                                                     => 'Cog register 3c9c',
			'You may also like…'                                                            => 'Cog register 043e',
			'Related products'                                                              => 'Shop-floor find 3ff5',
		);
...
// and callouts stay theme-distinct (Cog = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'cog_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'cog_render_account_dashboard' ) ) {
	/**
	 * Render the Cog-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Cog voice.
	 ...
	 */
	function cog_render_account_dashboard(): void {
		...
		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'cog' ),
				'title'   => __( 'Recent orders', 'cog' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'cog' ),
				'cta'     => __( 'View orders', 'cog' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'cog' ),
				'title'   => __( 'Saved addresses', 'cog' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'cog' ),
				'cta'     => __( 'Edit addresses', 'cog' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'cog' ),
				'title'   => __( 'Account details', 'cog' ),
				'lede'    => __( 'Edit your name, email, and password.', 'cog' ),
				'cta'     => __( 'Edit profile', 'cog' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'cog' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'cog' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'cog' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Cog') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Cog and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'cog' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Cog.', 'cog' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'cog' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'cog' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'cog' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'cog' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'cog' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'cog' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `cog_swatches_color_map()` so
...
if ( ! function_exists( 'cog_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function cog_swatches_render_group( string $default_html, array $args ): string {
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return cog_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'cog' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'cog' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'cog' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'cog' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'cog' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'cog' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'cog' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'cog' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'cog' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'cog' ); ?></a>
	</p>
</div>
				<?php
