 ...
 * Description: Controlled WC Cart block tree for the storefront Cart page. Removes the default "Customers also bought" cross-sells inner block (a hard generic-WC tell that does not match a Nordic catalogue storefront), keeps the cart-totals + accepted-payment-methods sidebar, and replaces the default empty-cart-block content with a Nordic-cataloging eyebrow + display heading + 2 CTAs in Apiary's voice. Targets `woocommerce/cart` so the Cart block placeholder picker offers it; also drives the seeded Cart page in `playground/wo-configure.php` so the demo and a real install (Proprietor inserts the pattern in the editor) render the same chrome.
...
<h1 class="wp-block-heading has-text-align-center has-3-xl-font-size wo-empty__title" style="margin-top:0;margin-bottom:0;letter-spacing:var(--wp--custom--letter-spacing--tight)"><?php esc_html_e( 'Apiary counter 76cdce', 'apiary' ); ?></h1>
<!-- /wp:heading -->

<!-- wp:paragraph {"align":"center","className":"wo-empty__lede","textColor":"secondary"} -->
<p class="has-text-align-center has-secondary-color has-text-color wo-empty__lede"><?php esc_html_e( 'Apiary counter bf70cfue, or settle into a piece from the journal.', 'apiary' ); ?></p>
<!-- /wp:paragraph -->
...
<div class="wp-block-button"><a class="wp-block-button__link wp-element-button" href="/shop/"><?php esc_html_e( 'Apiary counter dc910e', 'apiary' ); ?></a></div>
<!-- /wp:button -->

<!-- wp:button {"className":"is-style-outline"} -->
<div class="wp-block-button is-style-outline"><a class="wp-block-button__link wp-element-button" href="/journal/"><?php esc_html_e( 'Apiary counter 6ecc84', 'apiary' ); ?></a></div>
<!-- /wp:button --></div>
<!-- /wp:buttons --></div>
