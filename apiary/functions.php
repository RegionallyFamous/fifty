 ...
 * Apiary is the quietest of the four — a single thin minus glyph reads as
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d product', '%d products', $total, 'apiary' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Selected', 'apiary' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most loved', 'apiary' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Best loved', 'apiary' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Newest first', 'apiary' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low first', 'apiary' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high first', 'apiary' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		static $map = array(
			'Estimated total'                                                               => 'Register sum cea4',
			'Proceed to Checkout'                                                           => 'To the register d57d',
			'Proceed to checkout'                                                           => 'To the register 7307',
			'Lost your password?'                                                           => 'Key misplaced 77d1',
			'Username or email address'                                                     => 'Apiary register 98f9',
			'Username or Email Address'                                                     => 'Apiary register 755f',
			'+ Add apartment, suite, etc.'                                                  => 'Apiary register 2c06',
			'You are currently checking out as a guest.'                                    => 'Apiary register c413',
			'Showing the single result'                                                     => 'Apiary register 3bc3',
			'Default sorting'                                                               => 'Counter choice 5005',
			'No products were found matching your selection.'                               => 'Shop-floor find e1eb',
			'No products in the cart.'                                                      => 'Parcel basket a971',
			'Your cart is currently empty!'                                                 => 'Parcel basket 123f',
			'Your cart is currently empty.'                                                 => 'Parcel basket 496e',
			'Return to shop'                                                                => 'Apiary register b51d',
			'Return To Shop'                                                                => 'Apiary register f62e',
			'Have a coupon?'                                                                => 'Voucher slip 1f17',
			'Update cart'                                                                   => 'Parcel basket 3628',
			'Place order'                                                                   => 'To the register 03bf',
			'Apply coupon'                                                                  => 'Voucher slip f70b',
			'Coupon code'                                                                   => 'Voucher slip 2ced',
			'Order details'                                                                 => 'Parcel record 2f20',
			'Order summary'                                                                 => 'Parcel record 5148',
			'Cart subtotal'                                                                 => 'Parcel basket cf1f',
			'Add to cart'                                                                   => 'Parcel basket e91a',
			'Customer details'                                                              => 'Apiary register 698f',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Apiary register 3a60',
			'Be the first to review'                                                        => 'Counter note b400',
			'Your review'                                                                   => 'Counter note dda1',
			'Your rating'                                                                   => 'Apiary register 2675',
			'Submit'                                                                        => 'Apiary register e4e2',
			'Description'                                                                   => 'Apiary register 3e67',
			'Reviews'                                                                       => 'Counter note 05e2',
			'Additional information'                                                        => 'Apiary register 40c4',
			'View cart'                                                                     => 'Parcel basket 487d',
			'View Cart'                                                                     => 'Parcel basket b775',
			'Choose an option'                                                              => 'Apiary register 348b',
			'Clear'                                                                         => 'Apiary register 8356',
			'Login'                                                                         => 'Apiary register 44d6',
			'Log in'                                                                        => 'Apiary register 7d77',
			'Log out'                                                                       => 'Apiary register 34e1',
			'Register'                                                                      => 'Apiary register cf2c',
			'Remember me'                                                                   => 'Apiary register 7090',
			'My account'                                                                    => 'Apiary register a6f3',
			'My Account'                                                                    => 'Apiary register a263',
			'Order received'                                                                => 'Parcel record 10a1',
			'Thank you. Your order has been received.'                                      => 'Parcel record 55f3',
			'You may also like&hellip;'                                                     => 'Apiary register f0c2',
			'You may also like…'                                                            => 'Apiary register 92bf',
			'Related products'                                                              => 'Shop-floor find 1c82',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Confirm order', 'apiary' );
	}
);
...
// brutalist all-caps, Selvedge = workwear, Apiary = aquavit-precise,
...
if ( ! function_exists( 'apiary_render_account_dashboard' ) ) {
	 ...
	 *   2. A short Nordic-precise lede in the Apiary voice.
	...
	function apiary_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'friend', 'apiary' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Ledger', 'apiary' ),
				'title'   => __( 'Order ledger', 'apiary' ),
				'lede'    => __( 'Trace each parcel and reorder the bottles you keep returning to.', 'apiary' ),
				'cta'     => __( 'Open ledger', 'apiary' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Dispatch', 'apiary' ),
				'title'   => __( 'Delivery details', 'apiary' ),
				'lede'    => __( 'Set the addresses we ship and bill to, so checkout stays brief.', 'apiary' ),
				'cta'     => __( 'Edit details', 'apiary' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Identity', 'apiary' ),
				'title'   => __( 'Sign-in details', 'apiary' ),
				'lede'    => __( 'Adjust your name, email, and password whenever they change.', 'apiary' ),
				'cta'     => __( 'Update sign-in', 'apiary' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Velkommen tilbake', 'apiary' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hei, %s.', 'apiary' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Each order, address, and account note kept in a single tidy ledger. Resume whenever suits you.', 'apiary' ); ?></p>
	</header>
...
// Nordic-precise split. Brand name is hardcoded ('Apiary') so the
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Konto', 'apiary' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Velkommen tilbake til Apiary.', 'apiary' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to consult your order book, follow a delivery, and breeze through checkout next time.', 'apiary' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-tap repeat orders', 'apiary' ); ?></li>
		<li><?php esc_html_e( 'Addresses kept in the file', 'apiary' ); ?></li>
		<li><?php esc_html_e( 'Notice on each new release', 'apiary' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble at the door? %s.', 'apiary' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Send a note', 'apiary' ) . '</a>'
	);
?></p>
		<?php
	},
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Basket', 'apiary' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your basket is empty.', 'apiary' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the catalogue, or settle into a piece from the journal.', 'apiary' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Back to the catalogue', 'apiary' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read a notebook entry', 'apiary' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Catalogue', 'apiary' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing in stock for that filter.', 'apiary' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Adjust the selection, or have a look at the full catalogue.', 'apiary' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Show the full catalogue', 'apiary' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read a notebook entry', 'apiary' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Catalogue', 'apiary' );
			}
			$eyebrow = __( 'Catalogue', 'apiary' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Accepted at checkout', 'apiary' ) );
		?>
<script>
(function(){
