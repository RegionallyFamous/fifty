...
add_action(
	...
	static function (): void {
		$categories = array(
			'apiary'        => array(
				'label'       => __( 'Apiary', 'apiary' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'apiary' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'apiary' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'apiary' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'apiary' ),
				'description' => __( 'Full-width hero and banner patterns.', 'apiary' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'apiary' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'apiary' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'apiary' ),
				'description' => __( 'Social proof and customer quote patterns.', 'apiary' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'apiary' ),
				'description' => __( 'Footer layout patterns.', 'apiary' ),
			),
		);
...
/**
 * Harvest-price sale badge.
 *
 * WC ships a chirpy `<span class="onsale">Sale!</span>` on the product image.
 * Apiary's shopkeeper would chalk a small "harvest price" tag on the jar
 * rather than shout — a calm hexagon-framed pill says this batch is at a
 * batch price without insisting on it. Styling lives in theme.json ->
 * styles.css; the mark is U+25CF BLACK CIRCLE, set inside a honey-gold
 * pill so it reads as a sealing-wax dot.
 */
add_filter(
	'woocommerce_sale_flash',
	static function (): string {
		return '<span class="onsale">' . esc_html__( '●', 'apiary' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Apiary voice.
//
// Voice: small-batch-honey-artisan crossed with warm-indie-grocery —
// a farmer's market stall run by an excellent beekeeper who writes
// their own labels by hand. Warm, direct, no exclamation marks.
// Vocabulary leans on hive / colony / harvest / honey / comb / queen /
// forage / pantry / jar / batch / harvest-date. Avoid stock food
// clichés ("delicious", "yummy") — this producer respects the
// ingredient.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			/* translators: %d: number of jars in the current archive. */
			esc_html( _n( '%d jar on the shelf', '%d jars on the shelf', $total, 'apiary' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Beekeeper’s pick', 'apiary' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most jarred', 'apiary' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Highest notes', 'apiary' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Freshest harvest', 'apiary' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: everyday first', 'apiary' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: rarest first', 'apiary' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Apiary voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Running total',
			'Proceed to Checkout'                                                           => 'Pack my jars',
			'Proceed to checkout'                                                           => 'Pack my jars',
			'Lost your password?'                                                           => 'Mislaid your password?',
			'Username or email address'                                                     => 'Email on file',
			'Username or Email Address'                                                     => 'Email on file',
			'+ Add apartment, suite, etc.'                                                  => '+ Apartment, cottage, shed',
			'You are currently checking out as a guest.'                                    => 'Already a regular? Sign in and we’ll pre-fill your pantry.',
			'Showing the single result'                                                     => 'One jar on the shelf',
			'Default sorting'                                                               => 'Beekeeper’s pick',
			'No products were found matching your selection.'                               => 'Nothing in the hive for those filters.',
			'No products in the cart.'                                                      => 'The pantry is empty.',
			'Your cart is currently empty!'                                                 => 'The pantry is empty.',
			'Your cart is currently empty.'                                                 => 'The pantry is empty.',
			'Return to shop'                                                                => 'Back to the honey shelf',
			'Return To Shop'                                                                => 'Back to the honey shelf',
			'Have a coupon?'                                                                => 'Have a market voucher?',
			'Update cart'                                                                   => 'Retally the pantry',
			'Place order'                                                                   => 'Seal the batch',
			'Apply coupon'                                                                  => 'Redeem voucher',
			'Coupon code'                                                                   => 'Market voucher',
			'Order details'                                                                 => 'Batch details',
			'Order summary'                                                                 => 'Batch summary',
			'Cart subtotal'                                                                 => 'Pantry subtotal',
			'Add to cart'                                                                   => 'Add to pantry',
			'Customer details'                                                              => 'Your address on file',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Save these for the next visit.',
			'Be the first to review'                                                        => 'Be the first to leave a tasting note',
			'Your review'                                                                   => 'Tasting note',
			'Your rating'                                                                   => 'Stars out of five',
			'Submit'                                                                        => 'Post tasting note',
			'Description'                                                                   => 'Tasting notes',
			'Reviews'                                                                       => 'Tasting notes from the pantry',
			'Additional information'                                                        => 'Harvest log',
			'View cart'                                                                     => 'Open the pantry',
			'View Cart'                                                                     => 'Open the pantry',
			'Choose an option'                                                              => 'Pick a variety',
			'Clear'                                                                         => 'Start over',
			'Login'                                                                         => 'Sign in to the hive',
			'Log in'                                                                        => 'Sign in to the hive',
			'Log out'                                                                       => 'Close the hive',
			'Register'                                                                      => 'Join the hive',
			'Remember me'                                                                   => 'Keep me in the hive',
			'My account'                                                                    => 'Your pantry',
			'My Account'                                                                    => 'Your pantry',
			'Order received'                                                                => 'Batch sealed, thank you',
			'Thank you. Your order has been received.'                                      => 'Thank you. Your batch is on the bench, waiting for the courier.',
			'You may also like&hellip;'                                                     => 'Pairs nicely with',
			'You may also like…'                                                            => 'Pairs nicely with',
			'Related products'                                                              => 'From the same hive',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Running total', 'apiary' );
	}
);
...
add_filter(
	...
	static function (): string {
		return __( 'Seal the batch', 'apiary' );
	}
);
...
// and callouts stay theme-distinct (Apiary = small-batch-honey-artisan,
// Chonk = brutalist all-caps, Selvedge = workwear, Lysholm =
// aquavit-precise, Aero = sport/technical). Same structural hooks,
// different voice.
add_action(
	'init',
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'apiary_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'apiary_render_account_dashboard' ) ) {
	/**
	 * Render the Apiary-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short warm-beekeeper lede in the Apiary voice.
	 ...
	 */
	function apiary_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'neighbour', 'apiary' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Harvest log', 'apiary' ),
				'title'   => __( 'Past batches', 'apiary' ),
				'lede'    => __( 'Every batch you’ve sealed, with jar counts, harvest dates, and a one-tap reorder for the ones you keep coming back to.', 'apiary' ),
				'cta'     => __( 'Open the harvest log', 'apiary' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Dispatch', 'apiary' ),
				'title'   => __( 'Where to send the jars', 'apiary' ),
				'lede'    => __( 'Kept addresses for home and the cottage, so sealing a batch stays a minute’s work.', 'apiary' ),
				'cta'     => __( 'Update addresses', 'apiary' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Keyring', 'apiary' ),
				'title'   => __( 'Name, email, password', 'apiary' ),
				'lede'    => __( 'The three keys to the hive. Change any of them whenever you like.', 'apiary' ),
				'cta'     => __( 'Change keyring', 'apiary' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Back at the hive', 'apiary' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello again, %s.', 'apiary' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Every batch you’ve taken home, every address on file, every key on the ring — kept tidy in one corner of the apiary, waiting for whenever you pop back in.', 'apiary' ); ?></p>
	</header>
...
// warm-pantry split. Brand name is hardcoded ('Apiary') so the
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Your pantry', 'apiary' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to the apiary.', 'apiary' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to revisit past batches, follow a parcel on its way, and seal the next order in about a minute.', 'apiary' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-tap reorders of the jar you keep finishing', 'apiary' ); ?></li>
		<li><?php esc_html_e( 'Home and cottage addresses kept side by side', 'apiary' ); ?></li>
		<li><?php esc_html_e( 'A quiet note whenever a new harvest is bottled', 'apiary' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		/* translators: %s: contact link wrapping the words "tap on the window". */
		esc_html__( 'Hive key not fitting? %s.', 'apiary' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Tap on the window', 'apiary' ) . '</a>'
	);
?></p>
		<?php
	},
...
// generic WC `<select>` with butter-yellow-and-cream hex swatches
// (nod to the hive); the hidden select stays in the DOM as WC's
// source of truth and an inline JS shim forwards button clicks
// into its `change` event.
if ( ! function_exists( 'apiary_swatches_color_map' ) ) {
	function apiary_swatches_color_map(): array {
		return array(
			'wildflower'    => '#e8a638',
			'buckwheat'     => '#6b3d1f',
			'orange-blossom' => '#d98441',
			'linden'        => '#e2c77a',
			'clover'        => '#f0d88a',
			'heather'       => '#b07aa1',
			'amber'         => '#c98018',
...
if ( ! function_exists( 'apiary_swatches_render_group' ) ) {
	function apiary_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = apiary_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return apiary_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Pantry', 'apiary' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your pantry shelf is bare.', 'apiary' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Wander through the honey shelf, or sit with a dispatch from the field notes while you decide.', 'apiary' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Browse the honey shelf', 'apiary' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read a field note', 'apiary' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Honey shelf', 'apiary' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing on the shelf for those filters.', 'apiary' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Widen the forage, or step back to the full honey shelf and start over.', 'apiary' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See every jar on the shelf', 'apiary' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read a field note', 'apiary' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'The honey shelf', 'apiary' );
			}
			$eyebrow = __( 'Honey shelf', 'apiary' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Welcome at the counter', 'apiary' ) );
		?>
<script>
(function(){
