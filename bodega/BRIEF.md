...
This file was emitted by `bin/design.py` after cloning `lysholm` -> `bodega` and applying the spec's palette + font choices. Read it before you do anything else; it captures the design intent the spec encoded so you can write the per-theme microcopy block, restyle the templates that need it, and brief any product photography in the same voice.

## Tagline

> hand-painted awning, fluorescent price stickers, neon OPEN

## Voice

contemporary food store: warm, direct, no exclamation marks.

Write this voice into the `// === BEGIN wc microcopy ===` block in `bodega/functions.php`. Every theme's microcopy must read distinctly from every other theme's; `bin/check.py check_wc_microcopy_distinct_across_themes` enforces it. Crib the structure from any sibling theme's microcopy block, then rewrite every literal string in this voice.
...
point at `bodega/assets/fonts/<slug>-<weight>.woff2` — drop the files there):

- `Hand-painted sign` (display, weights 400, 700)
...
2. Drop product photographs as `bodega/playground/images/product-wo-*.jpg` 
...
5. Re-shoot screenshot.png: `python3 bin/build-theme-screenshots.py bodega`.
6. Snap baseline: `python3 bin/snap.py shoot bodega && \
   python3 bin/snap.py baseline bodega`.
7. Verify: `python3 bin/check.py bodega --quick` — fix every failure before
...
_Brief auto-generated for bodega by `bin/design.py`._
