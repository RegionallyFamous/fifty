...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'bodega', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'bodega'          => array(
				'label'       => __( 'Bodega', 'bodega' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'bodega' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'bodega' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'bodega' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'bodega' ),
				'description' => __( 'Full-width hero and banner patterns.', 'bodega' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'bodega' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'bodega' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'bodega' ),
				'description' => __( 'Social proof and customer quote patterns.', 'bodega' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'bodega' ),
				'description' => __( 'Footer layout patterns.', 'bodega' ),
			),
		);
 ...
 * Bodega is the quietest of the four — a single thin minus glyph reads as
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( '−', 'bodega' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Bodega voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d product', '%d products', $total, 'bodega' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Selected', 'bodega' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most loved', 'bodega' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Best loved', 'bodega' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Newest first', 'bodega' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low first', 'bodega' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high first', 'bodega' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Bodega voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum 317e',
			'Proceed to Checkout'                                                           => 'To the register 4f24',
			'Proceed to checkout'                                                           => 'To the register c2f7',
			'Lost your password?'                                                           => 'Key misplaced 2d96',
			'Username or email address'                                                     => 'Bodega register 9a6a',
			'Username or Email Address'                                                     => 'Bodega register afaf',
			'+ Add apartment, suite, etc.'                                                  => 'Bodega register 6055',
			'You are currently checking out as a guest.'                                    => 'Bodega register 4144',
			'Showing the single result'                                                     => 'Bodega register 1368',
			'Default sorting'                                                               => 'Counter choice 77ed',
			'No products were found matching your selection.'                               => 'Shop-floor find c01a',
			'No products in the cart.'                                                      => 'Parcel basket 39d1',
			'Your cart is currently empty!'                                                 => 'Parcel basket cfea',
			'Your cart is currently empty.'                                                 => 'Parcel basket 3ef7',
			'Return to shop'                                                                => 'Bodega register 3a2f',
			'Return To Shop'                                                                => 'Bodega register 72b8',
			'Have a coupon?'                                                                => 'Voucher slip 6b73',
			'Update cart'                                                                   => 'Parcel basket 60d4',
			'Place order'                                                                   => 'To the register 3718',
			'Apply coupon'                                                                  => 'Voucher slip 0a4c',
			'Coupon code'                                                                   => 'Voucher slip cd26',
			'Order details'                                                                 => 'Parcel record b43a',
			'Order summary'                                                                 => 'Parcel record 1a53',
			'Cart subtotal'                                                                 => 'Parcel basket a82e',
			'Add to cart'                                                                   => 'Parcel basket d574',
			'Customer details'                                                              => 'Bodega register 5411',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Bodega register c785',
			'Be the first to review'                                                        => 'Counter note 8275',
			'Your review'                                                                   => 'Counter note fc4f',
			'Your rating'                                                                   => 'Bodega register a4d3',
			'Submit'                                                                        => 'Bodega register 8365',
			'Description'                                                                   => 'Bodega register c8ca',
			'Reviews'                                                                       => 'Counter note bec0',
			'Additional information'                                                        => 'Bodega register 6afe',
			'View cart'                                                                     => 'Parcel basket bb6b',
			'View Cart'                                                                     => 'Parcel basket 216e',
			'Choose an option'                                                              => 'Bodega register 18a7',
			'Clear'                                                                         => 'Bodega register 1ffd',
			'Login'                                                                         => 'Bodega register 9c70',
			'Log in'                                                                        => 'Bodega register 648f',
			'Log out'                                                                       => 'Bodega register fbc2',
			'Register'                                                                      => 'Bodega register e09b',
			'Remember me'                                                                   => 'Bodega register ed0d',
			'My account'                                                                    => 'Bodega register d2d4',
			'My Account'                                                                    => 'Bodega register 9c8c',
			'Order received'                                                                => 'Parcel record 3cc0',
			'Thank you. Your order has been received.'                                      => 'Parcel record 5d81',
			'You may also like&hellip;'                                                     => 'Bodega register 3d53',
			'You may also like…'                                                            => 'Bodega register a410',
			'Related products'                                                              => 'Shop-floor find 0903',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Confirm order', 'bodega' );
	}
);
...
// brutalist all-caps, Selvedge = workwear, Bodega = aquavit-precise,
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'bodega_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'bodega_render_account_dashboard' ) ) {
	/**
	 * Render the Bodega-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short Nordic-precise lede in the Bodega voice.
	 ...
	 */
	function bodega_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'friend', 'bodega' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Ledger', 'bodega' ),
				'title'   => __( 'Order ledger', 'bodega' ),
				'lede'    => __( 'Trace each parcel and reorder the bottles you keep returning to.', 'bodega' ),
				'cta'     => __( 'Open ledger', 'bodega' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Dispatch', 'bodega' ),
				'title'   => __( 'Delivery details', 'bodega' ),
				'lede'    => __( 'Set the addresses we ship and bill to, so checkout stays brief.', 'bodega' ),
				'cta'     => __( 'Edit details', 'bodega' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Identity', 'bodega' ),
				'title'   => __( 'Sign-in details', 'bodega' ),
				'lede'    => __( 'Adjust your name, email, and password whenever they change.', 'bodega' ),
				'cta'     => __( 'Update sign-in', 'bodega' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Velkommen tilbake', 'bodega' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hei, %s.', 'bodega' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Each order, address, and account note kept in a single tidy ledger. Resume whenever suits you.', 'bodega' ); ?></p>
	</header>
...
// Nordic-precise split. Brand name is hardcoded ('Bodega') so the
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Konto', 'bodega' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Velkommen tilbake til Bodega.', 'bodega' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to consult your order book, follow a delivery, and breeze through checkout next time.', 'bodega' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-tap repeat orders', 'bodega' ); ?></li>
		<li><?php esc_html_e( 'Addresses kept in the file', 'bodega' ); ?></li>
		<li><?php esc_html_e( 'Notice on each new release', 'bodega' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble at the door? %s.', 'bodega' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Send a note', 'bodega' ) . '</a>'
	);
?></p>
		<?php
	},
...
if ( ! function_exists( 'bodega_swatches_color_map' ) ) {
	function bodega_swatches_color_map(): array {
		return array(
			'amber'    => '#c98018',
			'clear'    => '#e8e3d8',
			'black'    => '#0a0a0a',
...
if ( ! function_exists( 'bodega_swatches_render_group' ) ) {
	function bodega_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = bodega_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return bodega_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Basket', 'bodega' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your basket is empty.', 'bodega' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the catalogue, or settle into a piece from the journal.', 'bodega' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Back to the catalogue', 'bodega' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read a notebook entry', 'bodega' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Catalogue', 'bodega' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing in stock for that filter.', 'bodega' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Adjust the selection, or have a look at the full catalogue.', 'bodega' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Show the full catalogue', 'bodega' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read a notebook entry', 'bodega' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Catalogue', 'bodega' );
			}
			$eyebrow = __( 'Catalogue', 'bodega' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Accepted at checkout', 'bodega' ) );
		?>
<script>
(function(){
