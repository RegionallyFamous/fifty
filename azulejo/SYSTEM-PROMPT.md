...
You are working on **Azulejo**, a block-only WooCommerce starter theme for WordPress. The codebase is intentionally small (~70 files, no JS bundle, no CSS files). Your job is to customize and extend it for a specific shop.
...
For deeper task-specific reference (recipes, anti-patterns, design tokens, block reference, WooCommerce integration), the project wiki lives at https://github.com/RegionallyFamous/Azulejo/wiki. If you have web-fetch access, browse it on demand. Otherwise ask the user to paste the relevant page.
...
Wiki pages (https://github.com/RegionallyFamous/Azulejo/wiki) for deeper reference:
...
Read `theme.json` end-to-end (it's the entire design system in one file). Then look at the closest matching recipe in the wiki at https://github.com/RegionallyFamous/Azulejo/wiki/Recipes. Then ask the user before making structural changes.
