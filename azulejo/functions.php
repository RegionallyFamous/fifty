...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'azulejo', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'azulejo'          => array(
				'label'       => __( 'Azulejo', 'azulejo' ),
				'description' => __( 'Patterns native to the Azulejo atelier.', 'azulejo' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'azulejo' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'azulejo' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'azulejo' ),
				'description' => __( 'Full-width hero and banner patterns.', 'azulejo' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'azulejo' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'azulejo' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'azulejo' ),
				'description' => __( 'Social proof and customer quote patterns.', 'azulejo' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'azulejo' ),
				'description' => __( 'Footer layout patterns.', 'azulejo' ),
			),
		);
 ...
 * Azulejo paints a tiny ceramic-blue dot on every piece that has been
 * reduced — quiet, unshouted, the kind of mark a museum placard makes
 * beside a study piece. The pill styling itself lives in theme.json ->
 * styles.css; the glyph is U+25AA BLACK SMALL SQUARE so it reads as an
 * inked dot at every zoom level.
 */
add_filter(
	'woocommerce_sale_flash',
	static function (): string {
		return '<span class="onsale">' . esc_html__( '▪', 'azulejo' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Azulejo voice — Iberian heritage
// craft, reverent and measured, centuries-tense rather than present-tense.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			/* translators: %d: number of tile patterns in the current archive. */
			esc_html( _n( '%d pattern in the atelier', '%d patterns in the atelier', $total, 'azulejo' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Arranged by the atelier', 'azulejo' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most commissioned', 'azulejo' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Held in highest regard', 'azulejo' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Fresh from the kiln', 'azulejo' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price, ascending', 'azulejo' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price, descending', 'azulejo' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Azulejo voice. Iberian heritage craft: pattern /
		// tile / piece for product; atelier / oficina for shop; commission
		// for order; firing log for provenance. Long measured sentences,
		// no exclamation marks, no modern startup phrasing.
		static $map = array(
			'Estimated total'                                                               => 'Total, to the atelier',
			'Proceed to Checkout'                                                           => 'Commission these patterns',
			'Proceed to checkout'                                                           => 'Commission these patterns',
			'Lost your password?'                                                           => 'Passcode forgotten? Request a fresh one',
			'Username or email address'                                                     => 'Correspondence address',
			'Username or Email Address'                                                     => 'Correspondence address',
			'+ Add apartment, suite, etc.'                                                  => '+ Floor, atelier number, portaria',
			'You are currently checking out as a guest.'                                    => 'Already on our register? Sign in to recall your details.',
			'Showing the single result'                                                     => 'A single pattern, found',
			'Default sorting'                                                               => 'Arranged by the atelier',
			'No products were found matching your selection.'                               => 'No tiles answer to that combination just now.',
			'No products in the cart.'                                                      => 'Nothing set aside yet.',
			'Your cart is currently empty!'                                                 => 'Nothing set aside yet.',
			'Your cart is currently empty.'                                                 => 'Nothing set aside yet.',
			'Return to shop'                                                                => 'Return to the oficina',
			'Return To Shop'                                                                => 'Return to the oficina',
			'Have a coupon?'                                                                => 'Carrying an atelier voucher?',
			'Update cart'                                                                   => 'Revise what is reserved',
			'Place order'                                                                   => 'Commit the commission',
			'Apply coupon'                                                                  => 'Redeem',
			'Coupon code'                                                                   => 'Atelier voucher',
			'Order details'                                                                 => 'Commission record',
			'Order summary'                                                                 => 'Commission in summary',
			'Cart subtotal'                                                                 => 'Reserved, before carriage',
			'Add to cart'                                                                   => 'Set aside this pattern',
			'Customer details'                                                              => 'Commissioning party',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Keep these details on the register for next time.',
			'Be the first to review'                                                        => 'Be the first to inscribe a note',
			'Your review'                                                                   => 'Your note, for the register',
			'Your rating'                                                                   => 'Your appraisal',
			'Submit'                                                                        => 'Note for the firing log',
			'Description'                                                                   => 'On this pattern',
			'Reviews'                                                                       => 'Notes from the register',
			'Additional information'                                                        => 'Firing log and provenance',
			'View cart'                                                                     => 'See what is reserved',
			'View Cart'                                                                     => 'See what is reserved',
			'Choose an option'                                                              => 'Select from the atelier',
			'Clear'                                                                         => 'Set aside this selection',
			'Login'                                                                         => 'Enter the register',
			'Log in'                                                                        => 'Enter the register',
			'Log out'                                                                       => 'Take leave of the register',
			'Register'                                                                      => 'Open a register with the atelier',
			'Remember me'                                                                   => 'Keep me on the register',
			'My account'                                                                    => 'The register',
			'My Account'                                                                    => 'The register',
			'Order received'                                                                => 'Commission logged',
			'Thank you. Your order has been received.'                                      => 'With our thanks. The commission is noted in the firing log.',
			'You may also like&hellip;'                                                     => 'From the same hand',
			'You may also like…'                                                            => 'From the same hand',
			'Related products'                                                              => 'Kindred patterns',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Total, to the atelier', 'azulejo' );
	}
);
...
add_filter(
	...
	static function (): string {
		return __( 'Commit the commission', 'azulejo' );
	}
);
...
// and callouts stay theme-distinct (Azulejo = quiet/editorial, Chonk =
// brutalist all-caps, Selvedge = workwear, Azulejo = heritage-craft,
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'azulejo_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'azulejo_render_account_dashboard' ) ) {
	/**
	 * Render the Azulejo-branded My Account dashboard tab.
	 *
	 * Replaces WC's default 2-paragraph greeting with:
	 *   1. A display-font greeting in Portuguese, using the customer's
	 *      first name (or login name as a fallback).
	 *   2. A short heritage-craft lede in the Azulejo voice.
	 *   3. A 3-card quick-link grid linking to the commission register,
	 *      delivery details, and sign-in — the surfaces that justify
	 *      opening a register with the atelier.
	 *
	 ...
	 */
	function azulejo_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'friend', 'azulejo' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'The register', 'azulejo' ),
				'title'   => __( 'Your commissions', 'azulejo' ),
				'lede'    => __( 'Consult the firing log: every pattern you have set aside, tile by tile, from the first note to the parcel at your door.', 'azulejo' ),
				'cta'     => __( 'Open the register', 'azulejo' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Carriage', 'azulejo' ),
				'title'   => __( 'Addresses on file', 'azulejo' ),
				'lede'    => __( 'Keep the shipping and invoicing addresses current, so each commission travels without delay to the right portaria.', 'azulejo' ),
				'cta'     => __( 'Revise the addresses', 'azulejo' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'The atelier door', 'azulejo' ),
				'title'   => __( 'Your sign-in', 'azulejo' ),
				'lede'    => __( 'Adjust the name we inscribe on your commissions, the correspondence address, and the passcode that opens the register.', 'azulejo' ),
				'cta'     => __( 'Amend the details', 'azulejo' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Bem-vindo de volta ao atelier', 'azulejo' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Olá, %s.', 'azulejo' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Every commission, every address, every note inscribed in one quiet register. Return to it whenever the afternoon allows.', 'azulejo' ); ?></p>
	</header>
...
// heritage-craft split. Brand name is hardcoded ('Azulejo') so the
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'The register', 'azulejo' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to the oficina.', 'azulejo' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to consult the firing log, follow a pattern on its journey, and let the next commission begin without retelling the same details.', 'azulejo' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'Commission again in a single gesture', 'azulejo' ); ?></li>
		<li><?php esc_html_e( 'Carriage addresses held in the register', 'azulejo' ); ?></li>
		<li><?php esc_html_e( 'A quiet note when a new pattern leaves the kiln', 'azulejo' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		/* translators: %s: contact link wrapping the words "write a line to the oficina". */
		esc_html__( 'The door will not open? %s and we will attend.', 'azulejo' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Write a line to the oficina', 'azulejo' ) . '</a>'
	);
?></p>
		<?php
	},
...
// generic WC `<select>` with heritage-craft pill swatches; the
// hidden select stays in the DOM as WC's source of truth and an
// inline JS shim forwards button clicks into its `change` event.
if ( ! function_exists( 'azulejo_swatches_color_map' ) ) {
	function azulejo_swatches_color_map(): array {
		// Named after pigments used in Lisbon azulejaria: cobalto is
		// the signature cobalt blue, creme the unglazed tin base, ocre
		// the gold ochre outline, verde-musgo a mossy Sintra green,
		// indigo the deep tonal wash, manganês the manganese purple
		// used in 18th-century pieces, terracota the fired-clay back,
		// preto the ink lined border.
		return array(
			'cobalto'     => '#1e40af',
			'creme'       => '#fef3c7',
			'ocre'        => '#ca8a04',
...
if ( ! function_exists( 'azulejo_swatches_render_group' ) ) {
	function azulejo_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = azulejo_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return azulejo_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
// WC's default banners with a heritage-craft eyebrow + display
// heading + 2-CTA strip.
add_action(
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Reserved', 'azulejo' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Nothing has been set aside just yet.', 'azulejo' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Step back into the oficina and choose a pattern to reserve, or linger awhile with a note from the atelier journal.', 'azulejo' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Return to the oficina', 'azulejo' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read from the firing log', 'azulejo' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'The atelier', 'azulejo' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'No tiles answer to that combination just now.', 'azulejo' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Adjust the selection, or see what has lately come from the kiln in the full catalogue of patterns.', 'azulejo' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See every pattern in the atelier', 'azulejo' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read from the firing log', 'azulejo' ); ?></a>
	</p>
</div>
				<?php
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			$eyebrow = ( 'product_cat' === $term->taxonomy )
				? __( 'From the atelier', 'azulejo' )
				: __( 'From the firing log', 'azulejo' );
			...
			if ( '' === $title ) {
				$title = __( 'The atelier', 'azulejo' );
			}
			$eyebrow = __( 'The atelier', 'azulejo' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Honoured at the atelier door', 'azulejo' ) );
		?>
<script>
(function(){
