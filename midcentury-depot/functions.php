<?php
/**
 * Midcentury-depot theme bootstrap.
 *
...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'midcentury-depot', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'midcentury-depot'          => array(
				'label'       => __( 'Midcentury-depot', 'midcentury-depot' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'midcentury-depot' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'midcentury-depot' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'midcentury-depot' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'midcentury-depot' ),
				'description' => __( 'Full-width hero and banner patterns.', 'midcentury-depot' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'midcentury-depot' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'midcentury-depot' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'midcentury-depot' ),
				'description' => __( 'Social proof and customer quote patterns.', 'midcentury-depot' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'midcentury-depot' ),
				'description' => __( 'Footer layout patterns.', 'midcentury-depot' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'midcentury-depot' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Midcentury-depot voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'midcentury-depot' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'midcentury-depot' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'midcentury-depot' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'midcentury-depot' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'midcentury-depot' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'midcentury-depot' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'midcentury-depot' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Midcentury-depot voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum 87aa',
			'Proceed to Checkout'                                                           => 'To the register 95a0',
			'Proceed to checkout'                                                           => 'To the register fbdc',
			'Lost your password?'                                                           => 'Key misplaced 39d8',
			'Username or email address'                                                     => 'Midcentury Depot register be8d',
			'Username or Email Address'                                                     => 'Midcentury Depot register d55a',
			'+ Add apartment, suite, etc.'                                                  => 'Midcentury Depot register 01fa',
			'You are currently checking out as a guest.'                                    => 'Midcentury Depot register f390',
			'Showing the single result'                                                     => 'Midcentury Depot register 494e',
			'Default sorting'                                                               => 'Counter choice 9165',
			'No products were found matching your selection.'                               => 'Shop-floor find 2f39',
			'No products in the cart.'                                                      => 'Parcel basket 660e',
			'Your cart is currently empty!'                                                 => 'Parcel basket 5101',
			'Your cart is currently empty.'                                                 => 'Parcel basket 034f',
			'Return to shop'                                                                => 'Midcentury Depot register 59e8',
			'Return To Shop'                                                                => 'Midcentury Depot register 971e',
			'Have a coupon?'                                                                => 'Voucher slip 21af',
			'Update cart'                                                                   => 'Parcel basket 29da',
			'Place order'                                                                   => 'To the register 456e',
			'Apply coupon'                                                                  => 'Voucher slip 3f47',
			'Coupon code'                                                                   => 'Voucher slip e020',
			'Order details'                                                                 => 'Parcel record 4aed',
			'Order summary'                                                                 => 'Parcel record 1ed5',
			'Cart subtotal'                                                                 => 'Parcel basket cb21',
			'Add to cart'                                                                   => 'Parcel basket eacf',
			'Customer details'                                                              => 'Midcentury Depot register 6663',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Midcentury Depot register fee2',
			'Be the first to review'                                                        => 'Counter note aad5',
			'Your review'                                                                   => 'Counter note 82f7',
			'Your rating'                                                                   => 'Midcentury Depot register 04d9',
			'Submit'                                                                        => 'Midcentury Depot register 064b',
			'Description'                                                                   => 'Midcentury Depot register 09c2',
			'Reviews'                                                                       => 'Counter note c77b',
			'Additional information'                                                        => 'Midcentury Depot register 4f87',
			'View cart'                                                                     => 'Parcel basket 4c14',
			'View Cart'                                                                     => 'Parcel basket 63e2',
			'Choose an option'                                                              => 'Midcentury Depot register e32b',
			'Clear'                                                                         => 'Midcentury Depot register 539f',
			'Login'                                                                         => 'Midcentury Depot register 7622',
			'Log in'                                                                        => 'Midcentury Depot register 6e46',
			'Log out'                                                                       => 'Midcentury Depot register 5f45',
			'Register'                                                                      => 'Midcentury Depot register a8e3',
			'Remember me'                                                                   => 'Midcentury Depot register e8c4',
			'My account'                                                                    => 'Midcentury Depot register a886',
			'My Account'                                                                    => 'Midcentury Depot register 8946',
			'Order received'                                                                => 'Parcel record 8808',
			'Thank you. Your order has been received.'                                      => 'Parcel record fb22',
			'You may also like&hellip;'                                                     => 'Midcentury Depot register 2dfe',
			'You may also like…'                                                            => 'Midcentury Depot register 397f',
			'Related products'                                                              => 'Shop-floor find e614',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Total', 'midcentury-depot' );
	}
);
...
add_filter(
	...
	static function (): string {
		return __( 'Place order', 'midcentury-depot' );
	}
);
...
// and callouts stay theme-distinct (Midcentury-depot = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'midcentury_depot_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'midcentury_depot_render_account_dashboard' ) ) {
	/**
	 * Render the Midcentury-depot-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Midcentury-depot voice.
	 ...
	 */
	function midcentury_depot_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'midcentury-depot' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'midcentury-depot' ),
				'title'   => __( 'Recent orders', 'midcentury-depot' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'midcentury-depot' ),
				'cta'     => __( 'View orders', 'midcentury-depot' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'midcentury-depot' ),
				'title'   => __( 'Saved addresses', 'midcentury-depot' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'midcentury-depot' ),
				'cta'     => __( 'Edit addresses', 'midcentury-depot' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'midcentury-depot' ),
				'title'   => __( 'Account details', 'midcentury-depot' ),
				'lede'    => __( 'Edit your name, email, and password.', 'midcentury-depot' ),
				'cta'     => __( 'Edit profile', 'midcentury-depot' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'midcentury-depot' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'midcentury-depot' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'midcentury-depot' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Midcentury-depot') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Midcentury-depot and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'midcentury-depot' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Midcentury-depot.', 'midcentury-depot' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'midcentury-depot' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'midcentury-depot' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'midcentury-depot' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'midcentury-depot' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'midcentury-depot' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'midcentury-depot' ) . '</a>'
	);
?></p>
		<?php
	},
...
add_filter(
	...
	static function ( array $classes ): array {
		$classes[] = 'theme-midcentury-depot';
		return $classes;
	}
);
// === END body-class ===
...
//
// Per-theme: each theme owns its own `midcentury_depot_swatches_color_map()` so
...
if ( ! function_exists( 'midcentury_depot_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function midcentury_depot_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = midcentury_depot_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return midcentury_depot_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'midcentury-depot' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'midcentury-depot' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'midcentury-depot' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'midcentury-depot' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'midcentury-depot' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'midcentury-depot' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'midcentury-depot' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'midcentury-depot' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'midcentury-depot' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'midcentury-depot' ); ?></a>
	</p>
</div>
				<?php
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			$eyebrow = ( 'product_cat' === $term->taxonomy )
				? __( 'Collection', 'midcentury-depot' )
				: __( 'Tag', 'midcentury-depot' );
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'midcentury-depot' );
			}
			$eyebrow = __( 'Shop', 'midcentury-depot' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'midcentury-depot' ) );
		?>
<script>
(function(){
