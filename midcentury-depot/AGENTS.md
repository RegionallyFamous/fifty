# AGENTS.md — Midcentury-depot

> Midcentury-depot is one theme inside the **Fifty monorepo**. The monorepo lives one directory up; its `AGENTS.md` (`../AGENTS.md`) and `README.md` (`../README.md`) describe the layout and the rules that apply to every theme. Read this file for Midcentury-depot-specific rules. Tooling lives in `../bin/`.

Instructions for AI coding agents working on the Midcentury-depot theme. Read this file in full before making any changes. Human-oriented docs live in `README.md` and the [project wiki](https://github.com/RegionallyFamous/Fifty/wiki).
...
| `python3 ../bin/clone.py NEW_NAME` | Clone Midcentury-depot into a new theme folder, renaming all identifiers. |
...
Midcentury-depot is a block-only WooCommerce starter theme for WordPress. It is intended to be copied (use `python3 ../bin/clone.py NEW_NAME`) and then customized by editing `theme.json` and adding project-specific patterns. The framework itself is deliberately small.
...
10. **Shopper-facing brand lives in this theme, never in `../playground/`.** When this theme ships, none of the monorepo's `playground/` directory travels with it — that directory only exists to set up the WordPress Playground demo (WXR import, catalogue seed, demo cart pre-fill). Any filter or action that affects what a real shopper sees MUST live inside this theme: layout goes in `templates/` / `parts/` / `patterns/`, visual tokens go in `theme.json`, and every brand-affecting PHP override goes in `functions.php` between the relevant sentinel pair — `// === BEGIN wc microcopy ===` (gettext map, sort labels, result-count rewrite, required-field marker, pagination glyphs, cart / order-button labels, any `render_block_woocommerce/*` or `woocommerce_blocks_*` filter), `// === BEGIN swatches ===` (per-brand variation swatches: color map + render callback + `wp_footer` JS shim, all delegated through `woocommerce_dropdown_variation_attribute_options_html`), `// === BEGIN payment-icons ===` (cart + checkout `wp_footer` injection of the payment-method strip), `// === BEGIN my-account ===` (login-intro panel + perks list + 2-column wrapper grid via the `woocommerce_before_customer_login_form` / `_after_customer_login_form` actions), `// === BEGIN empty-states ===` (branded empty cart + no-products-found via `woocommerce_cart_is_empty` and `woocommerce_no_products_found`), `// === BEGIN archive-hero ===` (editorial archive header strip via `woocommerce_before_main_content`), and `// === BEGIN body-class ===` (the per-theme `theme-<slug>` body class via `body_class`). Each sentinel-bracketed block scopes to this theme's text domain and color palette so the override travels with the released theme: drop the directory into `wp-content/themes/` on a real install and every brand surface ships with it. `../bin/check.py`'s `check_no_brand_filters_in_playground` fails the build (a) if any `playground/*.php` registers any of those brand-affecting hooks (gettext family, the six WC pages hooks, `woocommerce_dropdown_variation_attribute_options_html`, `body_class`, `render_block_woocommerce/*`, `woocommerce_blocks_*`) without an explicit `defined('WO_*')` demo-only guard, AND (b) if any `playground/*.php` source contains a `wo-empty` / `wo-account-` / `wo-archive-hero` / `wo-swatch` / `wo-payment-icons` marker class — that would mean a mu-plugin runtime injection or a `wo-configure.php` HEREDOC is painting brand markup from outside the theme. `check_wc_microcopy_distinct_across_themes` fails if this theme's microcopy duplicates another theme's translation for the same WC default (allowlist in `../bin/wc_microcopy_universal.json` for genuine universals like "Subtotal" / "Login" / "Apply"). NEVER reach for a `woocommerce_*_shop_loop` echo when a `render_block_<block-name>` filter exists — those legacy actions fire both in the historical loop AND inside `wp:woocommerce/product-collection`'s server render, which double-paints the HTML and lands the second copy floating with no parent flex container. See root rules #10, #11 and #17 for the full contract.
...
| Variation swatches (color map + render callback + `wp_footer` JS shim) | `functions.php` between `// === BEGIN swatches ===` / `// === END swatches ===` (per-theme color map; never `playground/`) |
| Payment-icons strip below cart + checkout Place Order button | `functions.php` between `// === BEGIN payment-icons ===` / `// === END payment-icons ===` (per-theme SVG pills; never `playground/`) |
| My Account login intro panel + perks list + 2-column grid | `functions.php` between `// === BEGIN my-account ===` / `// === END my-account ===` (per-theme voice; never `playground/`) |
| Branded empty cart + branded no-products-found state | `functions.php` between `// === BEGIN empty-states ===` / `// === END empty-states ===` (per-theme voice; never `playground/`) |
| Editorial archive header strip on category / tag / shop archives | `functions.php` between `// === BEGIN archive-hero ===` / `// === END archive-hero ===` (per-theme voice; never `playground/`) |
| Per-theme `theme-<slug>` body class (so `body.theme-<slug>` rules in `styles.css` scope correctly) | `functions.php` between `// === BEGIN body-class ===` / `// === END body-class ===` (hardcoded slug; never `playground/`) |
| Translation strings | `languages/` |
| Project tooling (validators, clone script) | `bin/` |
| Copy-from stubs for new files | `_examples/` |
| Long-form docs for humans | The [project wiki](https://github.com/RegionallyFamous/Midcentury-depot/wiki). Do not commit Markdown docs into the repo (other than `README.md`, `INDEX.md`, `SYSTEM-PROMPT.md`, `AGENTS.md`, `CHANGELOG.md`). |
...
4. Set `Categories: midcentury-depot-store` for project-style patterns or another registered category.
...
### Cloning Midcentury-depot for a new project
...
For deeper reference, point the user (or read directly via `WebFetch` if you can) at the [project wiki](https://github.com/RegionallyFamous/Midcentury-depot/wiki):

| Topic | Wiki page |
|---|---|
| Project structure (every file, what it does) | [Project-Structure](https://github.com/RegionallyFamous/Midcentury-depot/wiki/Project-Structure) |
| Design tokens (which slug to use when) | [Design-Tokens](https://github.com/RegionallyFamous/Midcentury-depot/wiki/Design-Tokens) |
| Step-by-step recipes for common tasks | [Recipes](https://github.com/RegionallyFamous/Midcentury-depot/wiki/Recipes) |
| Bad-code/good-code pairs | [Anti-Patterns](https://github.com/RegionallyFamous/Midcentury-depot/wiki/Anti-Patterns) |
| Inventory of every block referenced | [Block-Reference](https://github.com/RegionallyFamous/Midcentury-depot/wiki/Block-Reference) |
| WooCommerce template guide | [WooCommerce-Integration](https://github.com/RegionallyFamous/Midcentury-depot/wiki/WooCommerce-Integration) |
| Architecture and philosophy | [Architecture](https://github.com/RegionallyFamous/Midcentury-depot/wiki/Architecture) |
| Style variation guide | [Style-Variations](https://github.com/RegionallyFamous/Midcentury-depot/wiki/Style-Variations) |
| Tooling reference | [Tooling](https://github.com/RegionallyFamous/Midcentury-depot/wiki/Tooling) |
...
4. For task-specific guidance, browse the [project wiki](https://github.com/RegionallyFamous/Midcentury-depot/wiki).
