...
This file is the canonical design rubric for the **agitprop** theme. It is read by [`bin/snap-vision-review.py`](../bin/snap-vision-review.py) and concatenated into the prompt for screenshot review.
...
- `accent` — saturated single hot color (the agitprop "scream" color)

**Forbidden uses**:
- Soft borders (color only) — agitprop uses 2px+ hard black borders by default; "subtle border" defeats the brand
- Drop-shadows replacing borders (agitprop uses HARD edges, not soft depth)
- Gradients
...
- `display` — heavy/black weight, used at LARGE sizes for hero and section openers (this is the agitprop signature)
- `sans` — body and UI
...
**Forbidden**:
- Display weight set in light/regular (defeats the agitprop voice)
- Heading sizes < 2× base in heroes (agitprop hero typography is loud or it's broken)
...
- "Quiet" sections — agitprop doesn't have quiet space, it has loud space and louder space
...
- More than one accent color region per viewport (agitprop picks one loud color per screen)
...
These document deliberate decisions in the shipped agitprop theme that
...
- **WooCommerce default product cards (no hard border)** are acceptable
  ...
  border rule applies to agitprop's *own* card and zone treatments
