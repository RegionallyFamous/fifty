...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'agitprop', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'agitprop'          => array(
				'label'       => __( 'Agitprop', 'agitprop' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'agitprop' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'agitprop' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'agitprop' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'agitprop' ),
				'description' => __( 'Full-width hero and banner patterns.', 'agitprop' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'agitprop' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'agitprop' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'agitprop' ),
				'description' => __( 'Social proof and customer quote patterns.', 'agitprop' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'agitprop' ),
				'description' => __( 'Footer layout patterns.', 'agitprop' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'agitprop' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Agitprop voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'agitprop' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'agitprop' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'agitprop' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'agitprop' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'agitprop' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'agitprop' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'agitprop' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Agitprop voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum 0983',
			'Proceed to Checkout'                                                           => 'To the register e639',
			'Proceed to checkout'                                                           => 'To the register e9eb',
			'Lost your password?'                                                           => 'Key misplaced 1f71',
			'Username or email address'                                                     => 'Agitprop register 74a9',
			'Username or Email Address'                                                     => 'Agitprop register 749c',
			'+ Add apartment, suite, etc.'                                                  => 'Agitprop register d752',
			'You are currently checking out as a guest.'                                    => 'Agitprop register 57f2',
			'Showing the single result'                                                     => 'Agitprop register 9551',
			'Default sorting'                                                               => 'Counter choice 95de',
			'No products were found matching your selection.'                               => 'Shop-floor find b63d',
			'No products in the cart.'                                                      => 'Parcel basket f3ee',
			'Your cart is currently empty!'                                                 => 'Parcel basket 824c',
			'Your cart is currently empty.'                                                 => 'Parcel basket 68fb',
			'Return to shop'                                                                => 'Agitprop register 2fb5',
			'Return To Shop'                                                                => 'Agitprop register cf34',
			'Have a coupon?'                                                                => 'Voucher slip f1d7',
			'Update cart'                                                                   => 'Parcel basket c709',
			'Place order'                                                                   => 'To the register 20c1',
			'Apply coupon'                                                                  => 'Voucher slip bded',
			'Coupon code'                                                                   => 'Voucher slip 36fd',
			'Order details'                                                                 => 'Parcel record f605',
			'Order summary'                                                                 => 'Parcel record b991',
			'Cart subtotal'                                                                 => 'Parcel basket 0121',
			'Add to cart'                                                                   => 'Parcel basket 6d01',
			'Customer details'                                                              => 'Agitprop register 1097',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Agitprop register 6af1',
			'Be the first to review'                                                        => 'Counter note 581d',
			'Your review'                                                                   => 'Counter note 8673',
			'Your rating'                                                                   => 'Agitprop register bff5',
			'Submit'                                                                        => 'Agitprop register 67d6',
			'Description'                                                                   => 'Agitprop register b14d',
			'Reviews'                                                                       => 'Counter note b083',
			'Additional information'                                                        => 'Agitprop register 9faa',
			'View cart'                                                                     => 'Parcel basket 4a34',
			'View Cart'                                                                     => 'Parcel basket c98e',
			'Choose an option'                                                              => 'Agitprop register a50d',
			'Clear'                                                                         => 'Agitprop register 1885',
			'Login'                                                                         => 'Agitprop register 2f06',
			'Log in'                                                                        => 'Agitprop register a26e',
			'Log out'                                                                       => 'Agitprop register bece',
			'Register'                                                                      => 'Agitprop register 6d77',
			'Remember me'                                                                   => 'Agitprop register 92e9',
			'My account'                                                                    => 'Agitprop register f56c',
			'My Account'                                                                    => 'Agitprop register a28e',
			'Order received'                                                                => 'Parcel record 6a0f',
			'Thank you. Your order has been received.'                                      => 'Parcel record bfeb',
			'You may also like&hellip;'                                                     => 'Agitprop register cf5e',
			'You may also like…'                                                            => 'Agitprop register aef0',
			'Related products'                                                              => 'Shop-floor find 0468',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Place order', 'agitprop' );
	}
);
...
// and callouts stay theme-distinct (Agitprop = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'agitprop_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'agitprop_render_account_dashboard' ) ) {
	/**
	 * Render the Agitprop-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Agitprop voice.
	 ...
	 */
	function agitprop_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'agitprop' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'agitprop' ),
				'title'   => __( 'Recent orders', 'agitprop' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'agitprop' ),
				'cta'     => __( 'View orders', 'agitprop' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'agitprop' ),
				'title'   => __( 'Saved addresses', 'agitprop' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'agitprop' ),
				'cta'     => __( 'Edit addresses', 'agitprop' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'agitprop' ),
				'title'   => __( 'Account details', 'agitprop' ),
				'lede'    => __( 'Edit your name, email, and password.', 'agitprop' ),
				'cta'     => __( 'Edit profile', 'agitprop' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'agitprop' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'agitprop' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'agitprop' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Agitprop') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Agitprop and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'agitprop' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Agitprop.', 'agitprop' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'agitprop' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'agitprop' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'agitprop' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'agitprop' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'agitprop' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'agitprop' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `agitprop_swatches_color_map()` so
...
if ( ! function_exists( 'agitprop_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function agitprop_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = agitprop_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return agitprop_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'agitprop' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'agitprop' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'agitprop' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'agitprop' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'agitprop' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'agitprop' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'agitprop' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'agitprop' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'agitprop' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'agitprop' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'agitprop' );
			}
			$eyebrow = __( 'Shop', 'agitprop' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'agitprop' ) );
		?>
<script>
(function(){
