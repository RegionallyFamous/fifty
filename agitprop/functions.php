 ...
 * Agitprop leans loud, so the badge stays loud — but as the brand-voice "SALE"
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'Sale', 'agitprop' ) . '</span>';
	}
);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'TOP PICKS', 'agitprop' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'BEST SELLERS', 'agitprop' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'HIGHEST RATED', 'agitprop' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'JUST IN', 'agitprop' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'PRICE ↑', 'agitprop' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'PRICE ↓', 'agitprop' );
		}
		return $options;
	}
);
...
// and callouts stay theme-distinct (Obel = quiet/editorial, Agitprop =
...
if ( ! function_exists( 'agitprop_render_account_dashboard' ) ) {
	 ...
	 *   2. A short brutalist lede in the Agitprop voice.
	...
	function agitprop_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'friend', 'agitprop' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'LEDGER', 'agitprop' ),
				'title'   => __( 'ORDER LOG', 'agitprop' ),
				'lede'    => __( 'Track shipments. Reorder the hits.', 'agitprop' ),
				'cta'     => __( 'OPEN LEDGER', 'agitprop' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'SHIP-TO', 'agitprop' ),
				'title'   => __( 'WHERE IT GOES', 'agitprop' ),
				'lede'    => __( 'Edit billing and delivery so checkout flies.', 'agitprop' ),
				'cta'     => __( 'EDIT THE MAP', 'agitprop' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'IDENTITY', 'agitprop' ),
				'title'   => __( 'THE FILE ON YOU', 'agitprop' ),
				'lede'    => __( 'Name, email, password. Keep them current.', 'agitprop' ),
				'cta'     => __( 'UPDATE FILE', 'agitprop' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'BACK OFFICE', 'agitprop' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'HEY, %s.', 'agitprop' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Receipts, addresses, the basics. Get in, do the thing, get out.', 'agitprop' ); ?></p>
	</header>
...
// brutalist split. Brand name is hardcoded ('Agitprop') rather than
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'ID', 'agitprop' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'WELCOME BACK TO CHONK.', 'agitprop' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to scan the order log, follow a parcel, and get out the door faster next time.', 'agitprop' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'TAP-TO-REORDER', 'agitprop' ); ?></li>
		<li><?php esc_html_e( 'ADDRESSES ON FILE', 'agitprop' ); ?></li>
		<li><?php esc_html_e( 'FIRST DIBS ON DROPS', 'agitprop' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( "CAN'T GET IN? %s.", 'agitprop' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'POKE US', 'agitprop' ) . '</a>'
	);
?></p>
		<?php
	},
...
<div class="wo-empty wo-empty--cart">
	...
	<h1 class="wo-empty__title"><?php esc_html_e( 'CART: EMPTY.', 'agitprop' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Hit the shop or grab something off the journal.', 'agitprop' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'BACK TO SHOP', 'agitprop' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'READ THE JOURNAL', 'agitprop' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	...
	<h2 class="wo-empty__title"><?php esc_html_e( 'NOTHING FITS THAT FILTER.', 'agitprop' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different cut, or scan the whole catalogue.', 'agitprop' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'EVERY PRODUCT', 'agitprop' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'READ THE JOURNAL', 'agitprop' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'PAYMENTS', 'agitprop' ) );
		?>
<script>
(function(){
