...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'basalt', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'basalt'          => array(
				'label'       => __( 'Basalt', 'basalt' ),
				'description' => __( 'Starter studio patterns.', 'basalt' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'basalt' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'basalt' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'basalt' ),
				'description' => __( 'Full-width hero and banner patterns.', 'basalt' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'basalt' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'basalt' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'basalt' ),
				'description' => __( 'Social proof and customer quote patterns.', 'basalt' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'basalt' ),
				'description' => __( 'Footer layout patterns.', 'basalt' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'basalt' ) . '</span>';
	}
);
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '[ %d piece ]', '[ %d pieces ]', $total, 'basalt' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Catalog order', 'basalt' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most bought', 'basalt' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Best marked', 'basalt' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Most recent', 'basalt' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price, low to high', 'basalt' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price, high to low', 'basalt' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Basalt voice. Direct, present-tense, short fragments.
		// Stoneware/studio vocabulary. Per-theme overrides ship in each
		...
		static $map = array(
			...
			'Proceed to checkout'                                                           => 'To checkout',
			'Lost your password?'                                                           => 'Password reset',
			'Username or email address'                                                     => 'Email',
			'Username or Email Address'                                                     => 'Email',
			'+ Add apartment, suite, etc.'                                                  => '+ Studio or apartment',
			'You are currently checking out as a guest.'                                    => 'Studio accounts ship faster.',
			'Showing the single result'                                                     => '01 piece',
			'Default sorting'                                                               => 'Catalog order',
			'No products were found matching your selection.'                               => 'No pieces match those filters.',
			'No products in the cart.'                                                      => 'Cart holds no pieces.',
			'Your cart is currently empty!'                                                 => 'Cart holds no pieces.',
			'Your cart is currently empty.'                                                 => 'Cart holds no pieces.',
			'Return to shop'                                                                => 'Back to catalog',
			'Return To Shop'                                                                => 'Back to catalog',
			'Have a coupon?'                                                                => 'Promotion code?',
			'Update cart'                                                                   => 'Update',
			'Place order'                                                                   => 'Send order',
			'Apply coupon'                                                                  => 'Apply',
			'Coupon code'                                                                   => 'Promotion code',
			'Order details'                                                                 => 'Order record',
			'Order summary'                                                                 => 'Totals',
			'Cart subtotal'                                                                 => 'Subtotal',
			'Add to cart'                                                                   => 'Add piece',
			'Customer details'                                                              => 'Your contact',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Keep my details.',
			...
			'Remember me'                                                                   => 'Remember this studio',
			'My account'                                                                    => 'Account',
			'My Account'                                                                    => 'Account',
			'Order received'                                                                => 'Order placed',
			'Thank you. Your order has been received.'                                      => 'Order placed. Shipping within two days.',
			'You may also like&hellip;'                                                     => 'Also in the catalog',
			'You may also like…'                                                            => 'Also in the catalog',
			'Related products'                                                              => 'Also consider',
		);
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'basalt_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'basalt_render_account_dashboard' ) ) {
	/**
	 * Render the Basalt-branded My Account dashboard tab.
	 *
	 * Replaces WC's default 2-paragraph greeting with a short greeting
	 * and a 3-card quick-link grid. Voice: direct, present-tense, stone
	 * studio. See the wc-microcopy block for the voice rules.
	 */
	function basalt_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'friend', 'basalt' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Orders', 'basalt' ),
				'title'   => __( 'Order record', 'basalt' ),
				'lede'    => __( 'Past orders. Tracking numbers. Reorder in one click.', 'basalt' ),
				'cta'     => __( 'Open records', 'basalt' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Addresses', 'basalt' ),
				'title'   => __( 'Shipping on file', 'basalt' ),
				'lede'    => __( 'Billing and shipping, ready at checkout.', 'basalt' ),
				'cta'     => __( 'Revise addresses', 'basalt' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'basalt' ),
				'title'   => __( 'Studio contact', 'basalt' ),
				'lede'    => __( 'Name, email, password.', 'basalt' ),
				'cta'     => __( 'Revise profile', 'basalt' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Account', 'basalt' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( '%s.', 'basalt' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Orders, addresses, contact. Plain record of the studio relationship.', 'basalt' ); ?></p>
	</header>
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Studio account', 'basalt' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Sign in to Basalt.', 'basalt' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Past orders, shipping on file, faster checkout.', 'basalt' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'Reorder in one click', 'basalt' ); ?></li>
		<li><?php esc_html_e( 'Addresses on file', 'basalt' ); ?></li>
		<li><?php esc_html_e( 'Early access to new firings', 'basalt' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		/* translators: %s: contact link wrapping the words "Write the studio". */
		esc_html__( 'Cannot sign in? %s.', 'basalt' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Write the studio', 'basalt' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `basalt_swatches_color_map()` so
...
if ( ! function_exists( 'basalt_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function basalt_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = basalt_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return basalt_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( '[ cart ]', 'basalt' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Cart holds no pieces.', 'basalt' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Open the catalog. Or read the studio notes.', 'basalt' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Open catalog', 'basalt' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read studio notes', 'basalt' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( '[ catalog ]', 'basalt' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'No pieces match those filters.', 'basalt' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Pick a different form. Or view the full catalog.', 'basalt' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Full catalog', 'basalt' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Studio notes', 'basalt' ); ?></a>
	</p>
</div>
				<?php
			},
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Catalog', 'basalt' );
			}
			$eyebrow = __( '[ catalog ]', 'basalt' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Payment methods', 'basalt' ) );
		?>
<script>
(function(){
