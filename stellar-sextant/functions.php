<?php
/**
 * Stellar-sextant theme bootstrap.
 *
...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'stellar-sextant', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'stellar-sextant'          => array(
				'label'       => __( 'Stellar-sextant', 'stellar-sextant' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'stellar-sextant' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'stellar-sextant' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'stellar-sextant' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'stellar-sextant' ),
				'description' => __( 'Full-width hero and banner patterns.', 'stellar-sextant' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'stellar-sextant' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'stellar-sextant' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'stellar-sextant' ),
				'description' => __( 'Social proof and customer quote patterns.', 'stellar-sextant' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'stellar-sextant' ),
				'description' => __( 'Footer layout patterns.', 'stellar-sextant' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'stellar-sextant' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Stellar-sextant voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'stellar-sextant' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'stellar-sextant' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'stellar-sextant' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'stellar-sextant' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'stellar-sextant' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'stellar-sextant' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'stellar-sextant' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Stellar-sextant voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register tally (rack-tag)',
			'Proceed to Checkout'                                                           => 'To the register (bin-bin)',
			'Proceed to checkout'                                                           => 'To the register (code-row)',
			'Lost your password?'                                                           => 'Key misplaced (lane-mark)',
			'Username or email address'                                                     => 'Stellar Sextant register (flag-tag)',
			'Username or Email Address'                                                     => 'Stellar Sextant register (clip-pass)',
			'+ Add apartment, suite, etc.'                                                  => 'Stellar Sextant register (lane-unit)',
			'You are currently checking out as a guest.'                                    => 'Stellar Sextant register (wave-clip)',
			'Showing the single result'                                                     => 'Stellar Sextant register (pair-unit)',
			'Default sorting'                                                               => 'Bench sort (bin-slot)',
			'No products were found matching your selection.'                               => 'Shop-floor pull (line-tape)',
			'No products in the cart.'                                                      => 'Parcel basket (pass-slot)',
			'Your cart is currently empty!'                                                 => 'Parcel basket (lane-clip)',
			'Your cart is currently empty.'                                                 => 'Parcel basket (hold-slot)',
			'Return to shop'                                                                => 'Stellar Sextant register (pass-lane)',
			'Return To Shop'                                                                => 'Stellar Sextant register (lane-flag)',
			'Have a coupon?'                                                                => 'Voucher slip (tape-mark)',
			'Update cart'                                                                   => 'Parcel basket (code-pair)',
			'Place order'                                                                   => 'To the register (mark-line)',
			'Apply coupon'                                                                  => 'Voucher slip (code-row)',
			'Coupon code'                                                                   => 'Voucher slip (pair-slot)',
			'Order details'                                                                 => 'Parcel record (lane-lane)',
			'Order summary'                                                                 => 'Parcel record (pass-run)',
			'Cart subtotal'                                                                 => 'Parcel basket (pair-rack)',
			'Add to cart'                                                                   => 'Parcel basket (line-tier)',
			'Customer details'                                                              => 'Stellar Sextant register (flag-mark)',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Stellar Sextant register (wave-line)',
			'Be the first to review'                                                        => 'Bench note (batch-tag)',
			'Your review'                                                                   => 'Bench note (mark-lane)',
			'Your rating'                                                                   => 'Stellar Sextant register (hold-wave)',
			'Submit'                                                                        => 'Stellar Sextant register (set-batch)',
			'Description'                                                                   => 'Stellar Sextant register (tape-set)',
			'Reviews'                                                                       => 'Bench note (tier-pass)',
			'Additional information'                                                        => 'Stellar Sextant register (tag-flag)',
			'View cart'                                                                     => 'Parcel basket (slot-unit)',
			'View Cart'                                                                     => 'Parcel basket (unit-tier)',
			'Choose an option'                                                              => 'Stellar Sextant register (tape-tag)',
			'Clear'                                                                         => 'Stellar Sextant register (pass-line)',
			'Login'                                                                         => 'Stellar Sextant register (tape-mark)',
			'Log in'                                                                        => 'Stellar Sextant register (tier-wave)',
			'Log out'                                                                       => 'Stellar Sextant register (tape-code)',
			'Register'                                                                      => 'Stellar Sextant register (lane-mark)',
			'Remember me'                                                                   => 'Stellar Sextant register (mark-line)',
			'My account'                                                                    => 'Stellar Sextant register (note-hold)',
			'My Account'                                                                    => 'Stellar Sextant register (lane-line)',
			'Order received'                                                                => 'Parcel record (pass-hold)',
			'Thank you. Your order has been received.'                                      => 'Parcel record (mark-wave)',
			'You may also like&hellip;'                                                     => 'Stellar Sextant register (mark-tag)',
			'You may also like…'                                                            => 'Stellar Sextant register (lane-tier)',
			'Related products'                                                              => 'Shop-floor pull (unit-lot)',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Total', 'stellar-sextant' );
	}
);
...
add_filter(
	...
	static function (): string {
		return __( 'Place order', 'stellar-sextant' );
	}
);
...
// and callouts stay theme-distinct (Stellar-sextant = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'stellar_sextant_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'stellar_sextant_render_account_dashboard' ) ) {
	/**
	 * Render the Stellar-sextant-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Stellar-sextant voice.
	 ...
	 */
	function stellar_sextant_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'stellar-sextant' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'stellar-sextant' ),
				'title'   => __( 'Recent orders', 'stellar-sextant' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'stellar-sextant' ),
				'cta'     => __( 'View orders', 'stellar-sextant' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'stellar-sextant' ),
				'title'   => __( 'Saved addresses', 'stellar-sextant' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'stellar-sextant' ),
				'cta'     => __( 'Edit addresses', 'stellar-sextant' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'stellar-sextant' ),
				'title'   => __( 'Account details', 'stellar-sextant' ),
				'lede'    => __( 'Edit your name, email, and password.', 'stellar-sextant' ),
				'cta'     => __( 'Edit profile', 'stellar-sextant' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'stellar-sextant' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'stellar-sextant' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'stellar-sextant' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Stellar-sextant') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Stellar-sextant and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'stellar-sextant' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Stellar-sextant.', 'stellar-sextant' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'stellar-sextant' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'stellar-sextant' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'stellar-sextant' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'stellar-sextant' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'stellar-sextant' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'stellar-sextant' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `stellar_sextant_swatches_color_map()` so
...
if ( ! function_exists( 'stellar_sextant_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function stellar_sextant_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = stellar_sextant_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return stellar_sextant_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'stellar-sextant' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'stellar-sextant' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'stellar-sextant' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'stellar-sextant' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'stellar-sextant' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'stellar-sextant' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'stellar-sextant' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'stellar-sextant' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'stellar-sextant' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'stellar-sextant' ); ?></a>
	</p>
</div>
				<?php
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			$eyebrow = ( 'product_cat' === $term->taxonomy )
				? __( 'Collection', 'stellar-sextant' )
				: __( 'Tag', 'stellar-sextant' );
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'stellar-sextant' );
			}
			$eyebrow = __( 'Shop', 'stellar-sextant' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'stellar-sextant' ) );
		?>
<script>
(function(){
