...
def _make_product_photo(
    product_name: str,
    slug: str,          # product slug (used for subtle differentiation)
    ...
    bar_h = max(8, size // 40)
    bar_rect = [margin + 2, margin + 2, size - margin - 2, margin + 2 + bar_h]
...
def _make_category_cover(
    ...
    bg = tuple(int(base[i] * 0.92 + accent[i] * 0.08) for i in range(3))
...
def generate_photos_for_theme(
    ...
    *,
    ...
    if written:
        cmd = [sys.executable, str(MONOREPO_ROOT / "bin" / "seed-playground-content.py"),
               "--theme", slug]
        if not quiet:
