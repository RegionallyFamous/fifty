...
This file was emitted by `bin/design.py` after cloning `obel` -> `agave` and applying the spec's palette + font choices. Read it before you do anything else; it captures the design intent the spec encoded so you can write the per-theme microcopy block, restyle the templates that need it, and brief any product photography in the same voice.

## Tagline

> cream paper with a 3-cell labelled plant-grid (Agave tequilana, Opuntia ficus-indica, Yucca filamentosa) in fine-line ink, terracotta wordmark band up top, sage + dust-pink captions, products presented as specimen cards in the same grid

## Voice

contemporary beauty store: warm, direct, no exclamation marks.

Write this voice into the `// === BEGIN wc microcopy ===` block in `agave/functions.php`. Every theme's microcopy must read distinctly from every other theme's; `bin/check.py check_wc_microcopy_distinct_across_themes` enforces it. Crib the structure from any sibling theme's microcopy block, then rewrite every literal string in this voice.
...
point at `agave/assets/fonts/<slug>-<weight>.woff2` — drop the files there):
...
2. Drop product photographs as `agave/playground/images/product-wo-*.jpg` 
...
5. Re-shoot screenshot.png: `python3 bin/build-theme-screenshots.py agave`.
6. Snap baseline: `python3 bin/snap.py shoot agave && \
   python3 bin/snap.py baseline agave`.
7. Verify: `python3 bin/check.py agave --quick` — fix every failure before
...
_Brief auto-generated for agave by `bin/design.py`._
