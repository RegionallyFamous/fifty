...
add_action(
	...
	static function (): void {
		$categories = array(
			'agave'          => array(
				'label'       => __( 'Agave', 'agave' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'agave' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'agave' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'agave' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'agave' ),
				'description' => __( 'Full-width hero and banner patterns.', 'agave' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'agave' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'agave' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'agave' ),
				'description' => __( 'Social proof and customer quote patterns.', 'agave' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'agave' ),
				'description' => __( 'Footer layout patterns.', 'agave' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'agave' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Agave voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d specimen in bloom', '%d specimens in bloom', $total, 'agave' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Curator\'s pick', 'agave' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most collected', 'agave' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Highest remarks', 'agave' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Newly catalogued', 'agave' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price, ascending', 'agave' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price, descending', 'agave' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Agave voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		//
		// Agave voice: a herbarium-adjacent beauty studio. Products are
		// "specimens", the shop is a "catalog" or "herbarium", orders are
		// "collected", the journal is a "field guide". Quiet, direct,
		// no exclamation marks; terracotta-ink-on-cream tone rather than
		// beauty-industry hype.
		static $map = array(
			'Estimated total'                                                               => 'Running sum',
			'Proceed to Checkout'                                                           => 'Bring to the counter',
			'Proceed to checkout'                                                           => 'Bring to the counter',
			'Lost your password?'                                                           => 'Forgotten passphrase',
			'Username or email address'                                                     => 'Email',
			'Username or Email Address'                                                     => 'Email',
			'+ Add apartment, suite, etc.'                                                  => 'Second line, if needed',
			'You are currently checking out as a guest.'                                    => 'Signed-in collectors autofill faster.',
			'Showing the single result'                                                     => 'One specimen',
			'Default sorting'                                                               => 'Curator\'s pick',
			'No products were found matching your selection.'                               => 'No specimens match that filter yet.',
			'No products in the cart.'                                                      => 'Nothing noted yet.',
			'Your cart is currently empty!'                                                 => 'Nothing noted yet.',
			'Your cart is currently empty.'                                                 => 'Nothing noted yet.',
			'Return to shop'                                                                => 'Back to the catalog',
			'Return To Shop'                                                                => 'Back to the catalog',
			'Have a coupon?'                                                                => 'Gift code',
			'Update cart'                                                                   => 'Update',
			'Place order'                                                                   => 'Collect the order',
			'Apply coupon'                                                                  => 'Apply',
			'Coupon code'                                                                   => 'Field code',
			'Order details'                                                                 => 'Ledger entry',
			'Order summary'                                                                 => 'Your specimens',
			'Cart subtotal'                                                                 => 'Subtotal',
			'Add to cart'                                                                   => 'Bottle one',
			'Customer details'                                                              => 'Delivery notes',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Remember my details for the next entry.',
			'Be the first to review'                                                        => 'Be the first to leave remarks',
			'Your review'                                                                   => 'Remarks',
			'Your rating'                                                                   => 'Rating',
			'Submit'                                                                        => 'Leave remarks',
			'Description'                                                                   => 'Description',
			'Reviews'                                                                       => 'Remarks',
			'Additional information'                                                        => 'Specimen notes',
			'View cart'                                                                     => 'View cart',
			'View Cart'                                                                     => 'View cart',
			'Choose an option'                                                              => 'Pick a cultivar',
			'Clear'                                                                         => 'Clear selection',
			'Login'                                                                         => 'Sign in',
			'Log in'                                                                        => 'Sign in',
			'Log out'                                                                       => 'Sign out',
			'Register'                                                                      => 'Open a ledger',
			'Remember me'                                                                   => 'Stay signed in on this device',
			'My account'                                                                    => 'Account',
			'My Account'                                                                    => 'Account',
			'Order received'                                                                => 'Collected',
			'Thank you. Your order has been received.'                                      => 'Noted. Your specimens are logged.',
			'You may also like&hellip;'                                                     => 'Related cultivars',
			'You may also like…'                                                            => 'Related cultivars',
			'Related products'                                                              => 'Related cultivars',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Collect the order', 'agave' );
	}
);
...
// and callouts stay theme-distinct (Agave = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'agave_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'agave_render_account_dashboard' ) ) {
	/**
	 * Render the Agave-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Agave voice.
	 ...
	 */
	function agave_render_account_dashboard(): void {
		...
		$cards = array(
			array(
				'eyebrow' => __( 'Ledger', 'agave' ),
				'title'   => __( 'Past collections', 'agave' ),
				'lede'    => __( 'Track parcels in transit and re-bottle a past specimen in a tap.', 'agave' ),
				'cta'     => __( 'Open the ledger', 'agave' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Delivery', 'agave' ),
				'title'   => __( 'Addresses on file', 'agave' ),
				'lede'    => __( 'Keep your billing and shipping details current so future collections move through quietly.', 'agave' ),
				'cta'     => __( 'Update addresses', 'agave' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Keeper', 'agave' ),
				'title'   => __( 'Account notes', 'agave' ),
				'lede'    => __( 'Change the name, email, or passphrase we hold on file for you.', 'agave' ),
				'cta'     => __( 'Amend details', 'agave' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Keeper', 'agave' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Good to see you again, %s.', 'agave' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Your field ledger — every specimen you have collected, every address on file, every note we hold — gathered in one place.', 'agave' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Agave') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Agave and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'The ledger', 'agave' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Sign back into the herbarium.', 'agave' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to revisit past collections, track what is in transit, and move through the counter in fewer steps next time.', 'agave' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'Re-bottle a past specimen in a tap.', 'agave' ); ?></li>
		<li><?php esc_html_e( 'Addresses and payment already on file.', 'agave' ); ?></li>
		<li><?php esc_html_e( 'First word on new cultivars in the catalog.', 'agave' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		/* translators: %s: contact link wrapping the words "Write to the studio". */
		esc_html__( 'Lost the passphrase? %s.', 'agave' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Write to the studio', 'agave' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `agave_swatches_color_map()` so
...
if ( ! function_exists( 'agave_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function agave_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = agave_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return agave_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Counter', 'agave' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Nothing set on the counter yet.', 'agave' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Wander the catalog, or pick up a thread in the field guide.', 'agave' ); ?></p>
	<p class="wo-empty__ctas">
			<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Open the catalog', 'agave' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Open the field guide', 'agave' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Catalog', 'agave' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'No specimens match that filter yet.', 'agave' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Loosen the cultivar set, or flip through the whole catalog.', 'agave' ); ?></p>
	<p class="wo-empty__ctas">
			<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Open the whole catalog', 'agave' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Open the field guide', 'agave' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'The catalog', 'agave' );
			}
			$eyebrow = __( 'The catalog', 'agave' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Payment accepted', 'agave' ) );
		?>
<script>
(function(){
