...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'distillery', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'distillery'          => array(
				'label'       => __( 'Distillery', 'distillery' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'distillery' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'distillery' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'distillery' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'distillery' ),
				'description' => __( 'Full-width hero and banner patterns.', 'distillery' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'distillery' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'distillery' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'distillery' ),
				'description' => __( 'Social proof and customer quote patterns.', 'distillery' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'distillery' ),
				'description' => __( 'Footer layout patterns.', 'distillery' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'distillery' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Distillery voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'distillery' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'distillery' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'distillery' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'distillery' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'distillery' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'distillery' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'distillery' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Distillery voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum d222',
			'Proceed to Checkout'                                                           => 'To the register 0858',
			'Proceed to checkout'                                                           => 'To the register e458',
			'Lost your password?'                                                           => 'Key misplaced de3a',
			'Username or email address'                                                     => 'Distillery register 4271',
			'Username or Email Address'                                                     => 'Distillery register 6084',
			'+ Add apartment, suite, etc.'                                                  => 'Distillery register a0a5',
			'You are currently checking out as a guest.'                                    => 'Distillery register 0f10',
			'Showing the single result'                                                     => 'Distillery register 4e11',
			'Default sorting'                                                               => 'Counter choice 173b',
			'No products were found matching your selection.'                               => 'Shop-floor find dfb2',
			'No products in the cart.'                                                      => 'Parcel basket 7d82',
			'Your cart is currently empty!'                                                 => 'Parcel basket e1ea',
			'Your cart is currently empty.'                                                 => 'Parcel basket 30e2',
			'Return to shop'                                                                => 'Distillery register d536',
			'Return To Shop'                                                                => 'Distillery register 8fe1',
			'Have a coupon?'                                                                => 'Voucher slip bbc6',
			'Update cart'                                                                   => 'Parcel basket 9346',
			'Place order'                                                                   => 'To the register 56cd',
			'Apply coupon'                                                                  => 'Voucher slip ee5c',
			'Coupon code'                                                                   => 'Voucher slip 9eb0',
			'Order details'                                                                 => 'Parcel record ef97',
			'Order summary'                                                                 => 'Parcel record a0ea',
			'Cart subtotal'                                                                 => 'Parcel basket 584b',
			'Add to cart'                                                                   => 'Parcel basket 7cd7',
			'Customer details'                                                              => 'Distillery register d2c5',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Distillery register 12dc',
			'Be the first to review'                                                        => 'Counter note df92',
			'Your review'                                                                   => 'Counter note 8dc0',
			'Your rating'                                                                   => 'Distillery register d382',
			'Submit'                                                                        => 'Distillery register cc40',
			'Description'                                                                   => 'Distillery register d177',
			'Reviews'                                                                       => 'Counter note 9acf',
			'Additional information'                                                        => 'Distillery register 6ee1',
			'View cart'                                                                     => 'Parcel basket ac12',
			'View Cart'                                                                     => 'Parcel basket 16ff',
			'Choose an option'                                                              => 'Distillery register a249',
			'Clear'                                                                         => 'Distillery register 01c9',
			'Login'                                                                         => 'Distillery register fbe0',
			'Log in'                                                                        => 'Distillery register fd5c',
			'Log out'                                                                       => 'Distillery register 41aa',
			'Register'                                                                      => 'Distillery register dda8',
			'Remember me'                                                                   => 'Distillery register 0135',
			'My account'                                                                    => 'Distillery register 5a50',
			'My Account'                                                                    => 'Distillery register 8e68',
			'Order received'                                                                => 'Parcel record fa43',
			'Thank you. Your order has been received.'                                      => 'Parcel record af46',
			'You may also like&hellip;'                                                     => 'Distillery register cb3c',
			'You may also like…'                                                            => 'Distillery register d5fd',
			'Related products'                                                              => 'Shop-floor find 5459',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Place order', 'distillery' );
	}
);
...
// and callouts stay theme-distinct (Distillery = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'distillery_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'distillery_render_account_dashboard' ) ) {
	/**
	 * Render the Distillery-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Distillery voice.
	 ...
	 */
	function distillery_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'distillery' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'distillery' ),
				'title'   => __( 'Recent orders', 'distillery' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'distillery' ),
				'cta'     => __( 'View orders', 'distillery' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'distillery' ),
				'title'   => __( 'Saved addresses', 'distillery' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'distillery' ),
				'cta'     => __( 'Edit addresses', 'distillery' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'distillery' ),
				'title'   => __( 'Account details', 'distillery' ),
				'lede'    => __( 'Edit your name, email, and password.', 'distillery' ),
				'cta'     => __( 'Edit profile', 'distillery' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'distillery' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'distillery' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'distillery' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Distillery') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Distillery and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'distillery' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Distillery.', 'distillery' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'distillery' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'distillery' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'distillery' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'distillery' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'distillery' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'distillery' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `distillery_swatches_color_map()` so
...
if ( ! function_exists( 'distillery_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function distillery_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = distillery_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return distillery_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'distillery' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'distillery' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'distillery' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'distillery' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'distillery' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'distillery' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'distillery' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'distillery' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'distillery' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'distillery' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'distillery' );
			}
			$eyebrow = __( 'Shop', 'distillery' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'distillery' ) );
		?>
<script>
(function(){
