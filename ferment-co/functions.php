...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'ferment-co', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'ferment-co'          => array(
				'label'       => __( 'Ferment-co', 'ferment-co' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'ferment-co' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'ferment-co' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'ferment-co' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'ferment-co' ),
				'description' => __( 'Full-width hero and banner patterns.', 'ferment-co' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'ferment-co' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'ferment-co' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'ferment-co' ),
				'description' => __( 'Social proof and customer quote patterns.', 'ferment-co' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'ferment-co' ),
				'description' => __( 'Footer layout patterns.', 'ferment-co' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'ferment-co' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Ferment-co voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'ferment-co' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'ferment-co' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'ferment-co' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'ferment-co' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'ferment-co' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'ferment-co' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'ferment-co' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Ferment-co voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register tally (flag-pass)',
			'Proceed to Checkout'                                                           => 'To the register (line-set)',
			'Proceed to checkout'                                                           => 'To the register (lot-hold)',
			'Lost your password?'                                                           => 'Key misplaced (tag-clip)',
			'Username or email address'                                                     => 'Ferment Co register (unit-clip)',
			'Username or Email Address'                                                     => 'Ferment Co register (tier-pass)',
			'+ Add apartment, suite, etc.'                                                  => 'Ferment Co register (hold-lane)',
			'You are currently checking out as a guest.'                                    => 'Ferment Co register (wave-hold)',
			'Showing the single result'                                                     => 'Ferment Co register (note-mark)',
			'Default sorting'                                                               => 'Bench sort (tape-unit)',
			'No products were found matching your selection.'                               => 'Shop-floor pull (pass-bin)',
			'No products in the cart.'                                                      => 'Parcel basket (note-batch)',
			'Your cart is currently empty!'                                                 => 'Parcel basket (slot-code)',
			'Your cart is currently empty.'                                                 => 'Parcel basket (wave-bin)',
			'Return to shop'                                                                => 'Ferment Co register (flag-lot)',
			'Return To Shop'                                                                => 'Ferment Co register (slot-flag)',
			'Have a coupon?'                                                                => 'Voucher slip (slot-pair)',
			'Update cart'                                                                   => 'Parcel basket (note-note)',
			'Place order'                                                                   => 'To the register (flag-row)',
			'Apply coupon'                                                                  => 'Voucher slip (tag-hold)',
			'Coupon code'                                                                   => 'Voucher slip (note-batch)',
			'Order details'                                                                 => 'Parcel record (run-hold)',
			'Order summary'                                                                 => 'Parcel record (lot-unit)',
			'Cart subtotal'                                                                 => 'Parcel basket (lane-pair)',
			'Add to cart'                                                                   => 'Parcel basket (rack-bin)',
			'Customer details'                                                              => 'Ferment Co register (pass-row)',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Ferment Co register (set-pair)',
			'Be the first to review'                                                        => 'Bench note (pair-set)',
			'Your review'                                                                   => 'Bench note (hold-line)',
			'Your rating'                                                                   => 'Ferment Co register (pair-row)',
			'Submit'                                                                        => 'Ferment Co register (rack-slot)',
			'Description'                                                                   => 'Ferment Co register (run-tape)',
			'Reviews'                                                                       => 'Bench note (pass-line)',
			'Additional information'                                                        => 'Ferment Co register (pair-rack)',
			'View cart'                                                                     => 'Parcel basket (pass-slot)',
			'View Cart'                                                                     => 'Parcel basket (pair-note)',
			'Choose an option'                                                              => 'Ferment Co register (clip-wave)',
			'Clear'                                                                         => 'Ferment Co register (flag-pass)',
			'Login'                                                                         => 'Ferment Co register (hold-set)',
			'Log in'                                                                        => 'Ferment Co register (mark-run)',
			'Log out'                                                                       => 'Ferment Co register (rack-line)',
			'Register'                                                                      => 'Ferment Co register (pair-set)',
			'Remember me'                                                                   => 'Ferment Co register (pass-run)',
			'My account'                                                                    => 'Ferment Co register (mark-bin)',
			'My Account'                                                                    => 'Ferment Co register (rack-tier)',
			'Order received'                                                                => 'Parcel record (tag-lot)',
			'Thank you. Your order has been received.'                                      => 'Parcel record (code-set)',
			'You may also like&hellip;'                                                     => 'Ferment Co register (tape-code)',
			'You may also like…'                                                            => 'Ferment Co register (mark-tape)',
			'Related products'                                                              => 'Shop-floor pull (code-clip)',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Place order', 'ferment-co' );
	}
);
...
// and callouts stay theme-distinct (Ferment-co = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'ferment_co_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'ferment_co_render_account_dashboard' ) ) {
	/**
	 * Render the Ferment-co-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Ferment-co voice.
	 ...
	 */
	function ferment_co_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'ferment-co' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'ferment-co' ),
				'title'   => __( 'Recent orders', 'ferment-co' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'ferment-co' ),
				'cta'     => __( 'View orders', 'ferment-co' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'ferment-co' ),
				'title'   => __( 'Saved addresses', 'ferment-co' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'ferment-co' ),
				'cta'     => __( 'Edit addresses', 'ferment-co' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'ferment-co' ),
				'title'   => __( 'Account details', 'ferment-co' ),
				'lede'    => __( 'Edit your name, email, and password.', 'ferment-co' ),
				'cta'     => __( 'Edit profile', 'ferment-co' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'ferment-co' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'ferment-co' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'ferment-co' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Ferment-co') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Ferment-co and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'ferment-co' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Ferment-co.', 'ferment-co' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'ferment-co' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'ferment-co' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'ferment-co' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'ferment-co' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'ferment-co' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'ferment-co' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `ferment_co_swatches_color_map()` so
...
if ( ! function_exists( 'ferment_co_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function ferment_co_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = ferment_co_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return ferment_co_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'ferment-co' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'ferment-co' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'ferment-co' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'ferment-co' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'ferment-co' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'ferment-co' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'ferment-co' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'ferment-co' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'ferment-co' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'ferment-co' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'ferment-co' );
			}
			$eyebrow = __( 'Shop', 'ferment-co' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'ferment-co' ) );
		?>
<script>
(function(){
