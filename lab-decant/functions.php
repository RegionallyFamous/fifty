...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'lab-decant', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'lab-decant'          => array(
				'label'       => __( 'Lab-decant', 'lab-decant' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'lab-decant' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'lab-decant' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'lab-decant' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'lab-decant' ),
				'description' => __( 'Full-width hero and banner patterns.', 'lab-decant' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'lab-decant' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'lab-decant' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'lab-decant' ),
				'description' => __( 'Social proof and customer quote patterns.', 'lab-decant' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'lab-decant' ),
				'description' => __( 'Footer layout patterns.', 'lab-decant' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'lab-decant' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Lab-decant voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'lab-decant' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'lab-decant' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'lab-decant' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'lab-decant' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'lab-decant' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'lab-decant' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'lab-decant' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Lab-decant voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register tally (hold-mark)',
			'Proceed to Checkout'                                                           => 'To the register (pass-bin)',
			'Proceed to checkout'                                                           => 'To the register (code-lot)',
			'Lost your password?'                                                           => 'Key misplaced (wave-lane)',
			'Username or email address'                                                     => 'Lab Decant register (tape-rack)',
			'Username or Email Address'                                                     => 'Lab Decant register (slot-mark)',
			'+ Add apartment, suite, etc.'                                                  => 'Lab Decant register (run-bin)',
			'You are currently checking out as a guest.'                                    => 'Lab Decant register (note-mark)',
			'Showing the single result'                                                     => 'Lab Decant register (pair-run)',
			'Default sorting'                                                               => 'Bench sort (hold-wave)',
			'No products were found matching your selection.'                               => 'Shop-floor pull (batch-tier)',
			'No products in the cart.'                                                      => 'Parcel basket (set-hold)',
			'Your cart is currently empty!'                                                 => 'Parcel basket (row-unit)',
			'Your cart is currently empty.'                                                 => 'Parcel basket (bin-hold)',
			'Return to shop'                                                                => 'Lab Decant register (batch-clip)',
			'Return To Shop'                                                                => 'Lab Decant register (batch-note)',
			'Have a coupon?'                                                                => 'Voucher slip (tape-flag)',
			'Update cart'                                                                   => 'Parcel basket (hold-pair)',
			'Place order'                                                                   => 'To the register (rack-slot)',
			'Apply coupon'                                                                  => 'Voucher slip (run-hold)',
			'Coupon code'                                                                   => 'Voucher slip (code-pair)',
			'Order details'                                                                 => 'Parcel record (code-pass)',
			'Order summary'                                                                 => 'Parcel record (bin-line)',
			'Cart subtotal'                                                                 => 'Parcel basket (bin-slot)',
			'Add to cart'                                                                   => 'Parcel basket (lot-note)',
			'Customer details'                                                              => 'Lab Decant register (batch-mark)',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Lab Decant register (slot-rack)',
			'Be the first to review'                                                        => 'Bench note (lane-batch)',
			'Your review'                                                                   => 'Bench note (flag-tape)',
			'Your rating'                                                                   => 'Lab Decant register (code-note)',
			'Submit'                                                                        => 'Lab Decant register (pair-pair)',
			'Description'                                                                   => 'Lab Decant register (tier-tag)',
			'Reviews'                                                                       => 'Bench note (hold-batch)',
			'Additional information'                                                        => 'Lab Decant register (unit-slot)',
			'View cart'                                                                     => 'Parcel basket (bin-pass)',
			'View Cart'                                                                     => 'Parcel basket (clip-hold)',
			'Choose an option'                                                              => 'Lab Decant register (clip-rack)',
			'Clear'                                                                         => 'Lab Decant register (lane-code)',
			'Login'                                                                         => 'Lab Decant register (unit-pair)',
			'Log in'                                                                        => 'Lab Decant register (run-row)',
			'Log out'                                                                       => 'Lab Decant register (wave-lot)',
			'Register'                                                                      => 'Lab Decant register (slot-pair)',
			'Remember me'                                                                   => 'Lab Decant register (pair-line)',
			'My account'                                                                    => 'Lab Decant register (lane-wave)',
			'My Account'                                                                    => 'Lab Decant register (mark-bin)',
			'Order received'                                                                => 'Parcel record (bin-pair)',
			'Thank you. Your order has been received.'                                      => 'Parcel record (code-row)',
			'You may also like&hellip;'                                                     => 'Lab Decant register (row-set)',
			'You may also like…'                                                            => 'Lab Decant register (wave-rack)',
			'Related products'                                                              => 'Shop-floor pull (bin-lane)',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Place order', 'lab-decant' );
	}
);
...
// and callouts stay theme-distinct (Lab-decant = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'lab_decant_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'lab_decant_render_account_dashboard' ) ) {
	/**
	 * Render the Lab-decant-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Lab-decant voice.
	 ...
	 */
	function lab_decant_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'lab-decant' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'lab-decant' ),
				'title'   => __( 'Recent orders', 'lab-decant' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'lab-decant' ),
				'cta'     => __( 'View orders', 'lab-decant' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'lab-decant' ),
				'title'   => __( 'Saved addresses', 'lab-decant' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'lab-decant' ),
				'cta'     => __( 'Edit addresses', 'lab-decant' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'lab-decant' ),
				'title'   => __( 'Account details', 'lab-decant' ),
				'lede'    => __( 'Edit your name, email, and password.', 'lab-decant' ),
				'cta'     => __( 'Edit profile', 'lab-decant' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'lab-decant' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'lab-decant' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'lab-decant' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Lab-decant') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Lab-decant and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'lab-decant' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Lab-decant.', 'lab-decant' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'lab-decant' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'lab-decant' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'lab-decant' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'lab-decant' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'lab-decant' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'lab-decant' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `lab_decant_swatches_color_map()` so
...
if ( ! function_exists( 'lab_decant_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function lab_decant_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = lab_decant_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return lab_decant_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'lab-decant' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'lab-decant' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'lab-decant' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'lab-decant' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'lab-decant' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'lab-decant' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'lab-decant' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'lab-decant' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'lab-decant' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'lab-decant' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'lab-decant' );
			}
			$eyebrow = __( 'Shop', 'lab-decant' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'lab-decant' ) );
		?>
<script>
(function(){
