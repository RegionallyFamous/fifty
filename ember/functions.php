...
add_action(
	...
	static function (): void {
		$categories = array(
			'ember'          => array(
				'label'       => __( 'Ember', 'ember' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'ember' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'ember' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'ember' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'ember' ),
				'description' => __( 'Full-width hero and banner patterns.', 'ember' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'ember' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'ember' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'ember' ),
				'description' => __( 'Social proof and customer quote patterns.', 'ember' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'ember' ),
				'description' => __( 'Footer layout patterns.', 'ember' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'ember' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Ember voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'ember' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'ember' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'ember' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'ember' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'ember' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'ember' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'ember' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Ember voice. Per-theme overrides ship in each
...
// and callouts stay theme-distinct (Ember = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'ember_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'ember_render_account_dashboard' ) ) {
	/**
	 * Render the Ember-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Ember voice.
	 ...
	 */
	function ember_render_account_dashboard(): void {
		...
		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'ember' ),
				'title'   => __( 'Recent orders', 'ember' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'ember' ),
				'cta'     => __( 'View orders', 'ember' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'ember' ),
				'title'   => __( 'Saved addresses', 'ember' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'ember' ),
				'cta'     => __( 'Edit addresses', 'ember' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'ember' ),
				'title'   => __( 'Account details', 'ember' ),
				'lede'    => __( 'Edit your name, email, and password.', 'ember' ),
				'cta'     => __( 'Edit profile', 'ember' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'ember' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'ember' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'ember' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Ember') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Ember and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'ember' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Ember.', 'ember' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'ember' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'ember' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'ember' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'ember' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'ember' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'ember' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `ember_swatches_color_map()` so
...
if ( ! function_exists( 'ember_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function ember_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = ember_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return ember_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'ember' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'ember' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'ember' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'ember' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'ember' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'ember' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'ember' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'ember' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'ember' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'ember' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'ember' ) );
		?>
<script>
(function(){
