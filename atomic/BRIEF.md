...
This file was emitted by `bin/design.py` after cloning `obel` -> `atomic` and applying the spec's palette + font choices. Read it before you do anything else; it captures the design intent the spec encoded so you can write the per-theme microcopy block, restyle the templates that need it, and brief any product photography in the same voice.

## Tagline

> hand-drawn starburst + boomerang + rocketship icons, mustard + teal + charcoal

## Voice

mid-century gift-shop store: warm, direct, no exclamation marks.
...
point at `atomic/assets/fonts/<slug>-<weight>.woff2` — drop the files there):

- `Futura Bold` (display, weights 400, 700)
