	...
	static function (): void {
		$categories = array(
			'atomic'          => array(
				...
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'atomic' ),
			),
			'woo-commerce'  => array(
			...
			'call-to-action' => array(
				...
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'atomic' ),
			),
			'testimonials'  => array(
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'atomic' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Atomic voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'atomic' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'atomic' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'atomic' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'atomic' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'atomic' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'atomic' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'atomic' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Atomic voice. Per-theme overrides ship in each
...
// and callouts stay theme-distinct (Atomic = quiet/editorial, Chonk =
...
if ( ! function_exists( 'atomic_render_account_dashboard' ) ) {
	/**
	 * Render the Atomic-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Atomic voice.
	...
	function atomic_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'atomic' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'atomic' ),
				'title'   => __( 'Recent orders', 'atomic' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'atomic' ),
				'cta'     => __( 'View orders', 'atomic' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'atomic' ),
				'title'   => __( 'Saved addresses', 'atomic' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'atomic' ),
				'cta'     => __( 'Edit addresses', 'atomic' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'atomic' ),
				'title'   => __( 'Account details', 'atomic' ),
				'lede'    => __( 'Edit your name, email, and password.', 'atomic' ),
				'cta'     => __( 'Edit profile', 'atomic' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'atomic' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'atomic' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'atomic' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Atomic') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Atomic and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'atomic' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Atomic.', 'atomic' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'atomic' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'atomic' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'atomic' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'atomic' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'atomic' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'atomic' ) . '</a>'
	);
?></p>
		<?php
	},
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'atomic' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'atomic' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'atomic' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'atomic' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'atomic' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'atomic' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'atomic' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'atomic' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'atomic' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'atomic' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'atomic' );
			}
			$eyebrow = __( 'Shop', 'atomic' );
		}
