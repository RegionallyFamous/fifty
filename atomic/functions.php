...
add_action(
	...
	static function (): void {
		$categories = array(
			'atomic'          => array(
				'label'       => __( 'Atomic', 'atomic' ),
				'description' => __( 'Patterns designed for the Atomic space-age storefront.', 'atomic' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'atomic' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'atomic' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'atomic' ),
				'description' => __( 'Full-width hero and banner patterns.', 'atomic' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'atomic' ),
				'description' => __( 'Conversion-focused banners and launch-log signups.', 'atomic' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'atomic' ),
				'description' => __( 'Social proof and customer quote patterns.', 'atomic' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'atomic' ),
				'description' => __( 'Footer layout patterns.', 'atomic' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'Launch deal', 'atomic' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Foundry voice.
//
...
add_filter(
	...
	static function ( array $args ): array {
		$args['prev_text'] = '&laquo; previous bay';
		$args['next_text'] = 'next bay &raquo;';
		return $args;
	}
);
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item in the capsule', '%d items in the capsule', $total, 'atomic' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Flight manifest', 'atomic' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most launched', 'atomic' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top of the console', 'atomic' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Fresh off the pad', 'atomic' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price &mdash; first stage up', 'atomic' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price &mdash; top stage down', 'atomic' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Atomic voice (1957 googie optimism; every
		// copy move points at liftoff / capsules / the flight deck,
		// without exclamation marks — the restraint is what keeps
		// the exuberance sophisticated). Per-theme overrides ship
		// in each theme's functions.php so divergence is visible to
		// the gate (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Payload total',
			'Proceed to Checkout'                                                           => 'To the launch pad',
			'Proceed to checkout'                                                           => 'To the launch pad',
			'Lost your password?'                                                           => 'Forgot the access code?',
			'Username or email address'                                                     => 'Pilot email',
			'Username or Email Address'                                                     => 'Pilot email',
			'+ Add apartment, suite, etc.'                                                  => 'Capsule, suite, or hangar no.',
			'You are currently checking out as a guest.'                                    => 'Flying solo today &mdash; a flight-deck account remembers your console for next launch.',
			'Showing the single result'                                                     => 'One item on the pad',
			'Default sorting'                                                               => 'Flight manifest',
			'No products were found matching your selection.'                               => 'The radar is clear &mdash; no capsules match those coordinates.',
			'No products in the cart.'                                                      => 'The capsule is still on the pad.',
			'Your cart is currently empty!'                                                 => 'The capsule is still on the pad.',
			'Your cart is currently empty.'                                                 => 'The capsule is still on the pad.',
			'Return to shop'                                                                => 'Back to the showroom',
			'Return To Shop'                                                                => 'Back to the showroom',
			'Have a coupon?'                                                                => 'Launch code on hand?',
			'Update cart'                                                                   => 'Recalculate trajectory',
			'Place order'                                                                   => 'Launch your order',
			'Apply coupon'                                                                  => 'Authorize',
			'Coupon code'                                                                   => 'Launch code',
			'Order details'                                                                 => 'Payload',
			'Order summary'                                                                 => 'Mission log',
			'Cart subtotal'                                                                 => 'Running payload',
			'Add to cart'                                                                   => 'Into the capsule',
			'Customer details'                                                              => 'Pilot particulars',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Save my console for the next orbit.',
			'Be the first to review'                                                        => 'Be the first to log a flight',
			'Your review'                                                                   => 'Flight report',
			'Your rating'                                                                   => 'Console rating',
			'Submit'                                                                        => 'Transmit',
			'Description'                                                                   => 'The specimen',
			'Reviews'                                                                       => 'Flight reports',
			'Additional information'                                                        => 'Spec card',
			'View cart'                                                                     => 'Inspect the capsule',
			'View Cart'                                                                     => 'Inspect the capsule',
			'Choose an option'                                                              => 'Select a configuration',
			'Clear'                                                                         => 'Reset the dial',
			'Login'                                                                         => 'Board the ship',
			'Log in'                                                                        => 'Board the ship',
			'Log out'                                                                       => 'Disembark',
			'Register'                                                                      => 'Commission a flight deck',
			'Remember me'                                                                   => 'Remember this console',
			'My account'                                                                    => 'Flight deck',
			'My Account'                                                                    => 'Flight deck',
			'Order received'                                                                => 'Payload cleared for launch',
			'Thank you. Your order has been received.'                                      => 'Thank you &mdash; your capsule is packed and the launch sequence is running.',
			'You may also like&hellip;'                                                     => 'Also in the galaxy',
			'You may also like…'                                                            => 'Also in the galaxy',
			'Related products'                                                              => 'Orbiting nearby',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Payload total', 'atomic' );
	}
);
...
add_filter(
	...
	static function (): string {
		return __( 'Launch your order', 'atomic' );
	}
);
...
// and callouts stay theme-distinct (Foundry = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'atomic_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'atomic_render_account_dashboard' ) ) {
	/**
	 * Render the Foundry-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Foundry voice.
	 ...
	 */
	function atomic_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'pilot', 'atomic' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Launch log', 'atomic' ),
				'title'   => __( 'Recent missions', 'atomic' ),
				'lede'    => __( 'Track every capsule in flight &mdash; and re-launch the ones you loved.', 'atomic' ),
				'cta'     => __( 'Open the log', 'atomic' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Trajectories', 'atomic' ),
				'title'   => __( 'Saved coordinates', 'atomic' ),
				'lede'    => __( 'Update your destination and billing console for a cleaner countdown.', 'atomic' ),
				'cta'     => __( 'Adjust coordinates', 'atomic' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Pilot card', 'atomic' ),
				'title'   => __( 'Crew manifest', 'atomic' ),
				'lede'    => __( 'Edit your call sign, communication channel, and access code.', 'atomic' ),
				'cta'     => __( 'Update manifest', 'atomic' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back to the flight deck', 'atomic' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Good orbit, %s.', 'atomic' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Every launch, every capsule, every console you&rsquo;ve dialled in &mdash; all on the same deck. Pick up the countdown where you left it.', 'atomic' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Foundry') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Foundry and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Flight deck', 'atomic' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back, pilot.', 'atomic' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Board the ship to review past missions, track capsules in transit, and cut your next countdown in half.', 'atomic' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-tap re-launch for favourites', 'atomic' ); ?></li>
		<li><?php esc_html_e( 'Saved coordinates + payment console', 'atomic' ); ?></li>
		<li><?php esc_html_e( 'Early access to mission-critical drops', 'atomic' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Console not responding? %s.', 'atomic' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Hail ground control', 'atomic' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `atomic_swatches_color_map()` so
...
if ( ! function_exists( 'atomic_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function atomic_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = atomic_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return atomic_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Capsule', 'atomic' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your capsule is sitting on the pad.', 'atomic' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Load a few wonders from the showroom, or wander the journal for launch inspiration.', 'atomic' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Tour the showroom', 'atomic' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read the mission log', 'atomic' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Showroom', 'atomic' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'The radar is clear on that frequency.', 'atomic' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Spin the dial to a different category, or browse every capsule currently in orbit.', 'atomic' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Browse the galaxy', 'atomic' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read the mission log', 'atomic' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'The showroom', 'atomic' );
			}
			$eyebrow = __( 'The collective', 'atomic' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'atomic' ) );
		?>
<script>
(function(){
