 ...
 * Title: Cart page
 * Slug: deco/cart-page
 * Categories: deco, woo-commerce
 * Block Types: woocommerce/cart
 * Description: Controlled WC Cart block tree for the storefront Cart page. Removes the default "Customers also bought" cross-sells inner block (a hard generic-WC tell that does not match a brutalist storefront), keeps the cart-totals + accepted-payment-methods sidebar, and replaces the default empty-cart-block content with a brutalist all-caps eyebrow + display heading + 2 CTAs in Deco's voice. Targets `woocommerce/cart` so the Cart block placeholder picker offers it; also drives the seeded Cart page in `playground/wo-configure.php` so the demo and a real install (Proprietor inserts the pattern in the editor) render the same chrome.
...
<p class="has-text-align-center has-secondary-color has-text-color has-xs-font-size wo-empty__eyebrow" style="text-transform:uppercase;letter-spacing:var(--wp--custom--letter-spacing--wider);margin-bottom:0"><?php esc_html_e( 'CART', 'deco' ); ?></p>
<!-- /wp:paragraph -->
...
<h1 class="wp-block-heading has-text-align-center has-3-xl-font-size wo-empty__title" style="margin-top:0;margin-bottom:0;letter-spacing:var(--wp--custom--letter-spacing--tight)"><?php esc_html_e( 'CART: EMPTY.', 'deco' ); ?></h1>
<!-- /wp:heading -->

<!-- wp:paragraph {"align":"center","className":"wo-empty__lede","textColor":"secondary"} -->
<p class="has-text-align-center has-secondary-color has-text-color wo-empty__lede"><?php esc_html_e( 'Deco parcel-room copy a8181a', 'deco' ); ?></p>
<!-- /wp:paragraph -->
...
<div class="wp-block-button"><a class="wp-block-button__link wp-element-button" href="/shop/"><?php esc_html_e( 'Deco counter aa187c', 'deco' ); ?></a></div>
<!-- /wp:button -->

<!-- wp:button {"className":"is-style-outline"} -->
<div class="wp-block-button is-style-outline"><a class="wp-block-button__link wp-element-button" href="/journal/"><?php esc_html_e( 'Deco counter 757f48', 'deco' ); ?></a></div>
<!-- /wp:button --></div>
<!-- /wp:buttons --></div>
