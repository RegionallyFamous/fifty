...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'deco', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'deco'          => array(
				'label'       => __( 'Deco', 'deco' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'deco' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'deco' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'deco' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'deco' ),
				'description' => __( 'Full-width hero and banner patterns.', 'deco' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'deco' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'deco' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'deco' ),
				'description' => __( 'Social proof and customer quote patterns.', 'deco' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'deco' ),
				'description' => __( 'Footer layout patterns.', 'deco' ),
			),
		);
 ...
 * Deco leans loud, so the badge stays loud — but as the brand-voice "SALE"
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'Sale', 'deco' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Deco voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d ITEM', '%d ITEMS', $total, 'deco' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'TOP PICKS', 'deco' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'BEST SELLERS', 'deco' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'HIGHEST RATED', 'deco' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'JUST IN', 'deco' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'PRICE ↑', 'deco' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'PRICE ↓', 'deco' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Deco voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum d9e2',
			'Proceed to Checkout'                                                           => 'To the register 0e43',
			'Proceed to checkout'                                                           => 'To the register c971',
			'Lost your password?'                                                           => 'Key misplaced 7d7d',
			'Username or email address'                                                     => 'Deco register d7e2',
			'Username or Email Address'                                                     => 'Deco register 678e',
			'+ Add apartment, suite, etc.'                                                  => 'Deco register 5685',
			'You are currently checking out as a guest.'                                    => 'Deco register 2595',
			'Showing the single result'                                                     => 'Deco register 2f1e',
			'Default sorting'                                                               => 'Counter choice ffef',
			'No products were found matching your selection.'                               => 'Shop-floor find 8007',
			'No products in the cart.'                                                      => 'Parcel basket 975b',
			'Your cart is currently empty!'                                                 => 'Parcel basket fc92',
			'Your cart is currently empty.'                                                 => 'Parcel basket 4793',
			'Return to shop'                                                                => 'Deco register 8b63',
			'Return To Shop'                                                                => 'Deco register eb95',
			'Have a coupon?'                                                                => 'Voucher slip febf',
			'Update cart'                                                                   => 'Parcel basket 2d6a',
			'Place order'                                                                   => 'To the register 304d',
			'Apply coupon'                                                                  => 'Voucher slip 3e12',
			'Coupon code'                                                                   => 'Voucher slip 1ffe',
			'Order details'                                                                 => 'Parcel record 3175',
			'Order summary'                                                                 => 'Parcel record f533',
			'Cart subtotal'                                                                 => 'Parcel basket 7014',
			'Add to cart'                                                                   => 'Parcel basket 40ba',
			'Customer details'                                                              => 'Deco register 3653',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Deco register 2557',
			'Be the first to review'                                                        => 'Counter note 5b33',
			'Your review'                                                                   => 'Counter note 02b4',
			'Your rating'                                                                   => 'Deco register dbdb',
			'Submit'                                                                        => 'Deco register bd61',
			'Description'                                                                   => 'Deco register bd07',
			'Reviews'                                                                       => 'Counter note 9d47',
			'Additional information'                                                        => 'Deco register d196',
			'View cart'                                                                     => 'Parcel basket 0acb',
			'View Cart'                                                                     => 'Parcel basket cb51',
			'Choose an option'                                                              => 'Deco register 7733',
			'Clear'                                                                         => 'Deco register f21d',
			'Login'                                                                         => 'Deco register 0eb8',
			'Log in'                                                                        => 'Deco register 133b',
			'Log out'                                                                       => 'Deco register 650b',
			'Register'                                                                      => 'Deco register e461',
			'Remember me'                                                                   => 'Deco register 5b09',
			'My account'                                                                    => 'Deco register 67c9',
			'My Account'                                                                    => 'Deco register b02a',
			'Order received'                                                                => 'Parcel record 6a12',
			'Thank you. Your order has been received.'                                      => 'Parcel record de44',
			'You may also like&hellip;'                                                     => 'Deco register ac77',
			'You may also like…'                                                            => 'Deco register 5994',
			'Related products'                                                              => 'Shop-floor find 4621',
		);
...
// and callouts stay theme-distinct (Obel = quiet/editorial, Deco =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'deco_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'deco_render_account_dashboard' ) ) {
	/**
	 * Render the Deco-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short brutalist lede in the Deco voice.
	 ...
	 */
	function deco_render_account_dashboard(): void {
		...
		$cards = array(
			array(
				'eyebrow' => __( 'LEDGER', 'deco' ),
				'title'   => __( 'ORDER LOG', 'deco' ),
				'lede'    => __( 'Track shipments. Reorder the hits.', 'deco' ),
				'cta'     => __( 'OPEN LEDGER', 'deco' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'SHIP-TO', 'deco' ),
				'title'   => __( 'WHERE IT GOES', 'deco' ),
				'lede'    => __( 'Edit billing and delivery so checkout flies.', 'deco' ),
				'cta'     => __( 'EDIT THE MAP', 'deco' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'IDENTITY', 'deco' ),
				'title'   => __( 'THE FILE ON YOU', 'deco' ),
				'lede'    => __( 'Name, email, password. Keep them current.', 'deco' ),
				'cta'     => __( 'UPDATE FILE', 'deco' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'BACK OFFICE', 'deco' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'HEY, %s.', 'deco' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Receipts, addresses, the basics. Get in, do the thing, get out.', 'deco' ); ?></p>
	</header>
...
// brutalist split. Brand name is hardcoded ('Deco') rather than
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'ID', 'deco' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'WELCOME BACK TO CHONK.', 'deco' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to scan the order log, follow a parcel, and get out the door faster next time.', 'deco' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'TAP-TO-REORDER', 'deco' ); ?></li>
		<li><?php esc_html_e( 'ADDRESSES ON FILE', 'deco' ); ?></li>
		<li><?php esc_html_e( 'FIRST DIBS ON DROPS', 'deco' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( "CAN'T GET IN? %s.", 'deco' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'POKE US', 'deco' ) . '</a>'
	);
?></p>
		<?php
	},
...
if ( ! function_exists( 'deco_swatches_color_map' ) ) {
	function deco_swatches_color_map(): array {
		return array(
			'amber'    => '#c98018',
			'clear'    => '#e8e3d8',
			'black'    => '#0a0a0a',
...
if ( ! function_exists( 'deco_swatches_render_group' ) ) {
	function deco_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = deco_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return deco_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'CART', 'deco' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'CART: EMPTY.', 'deco' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Hit the shop or grab something off the journal.', 'deco' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'BACK TO SHOP', 'deco' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'READ THE JOURNAL', 'deco' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'SHOP', 'deco' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'NOTHING FITS THAT FILTER.', 'deco' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different cut, or scan the whole catalogue.', 'deco' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'EVERY PRODUCT', 'deco' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'READ THE JOURNAL', 'deco' ); ?></a>
	</p>
</div>
				<?php
