...
```
https://playground.wordpress.net/?blueprint-url=https://raw.githubusercontent.com/RegionallyFamous/fifty/main/obel/playground/blueprint.json
```

The blueprint lives at [`obel/playground/blueprint.json`](playground/blueprint.json). It requires network access to fetch the theme zip and the dataset, so expect a 60–90 second startup. Once loaded you land on `/shop/` with all 30 products and images in place.
