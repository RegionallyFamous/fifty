...
add_action(
	...
	static function (): void {
		$categories = array(
			'bauhaus'          => array(
				'label'       => __( 'Bauhaus', 'bauhaus' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'bauhaus' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'bauhaus' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'bauhaus' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'bauhaus' ),
				'description' => __( 'Full-width hero and banner patterns.', 'bauhaus' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'bauhaus' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'bauhaus' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'bauhaus' ),
				'description' => __( 'Social proof and customer quote patterns.', 'bauhaus' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'bauhaus' ),
				'description' => __( 'Footer layout patterns.', 'bauhaus' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'bauhaus' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Bauhaus voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'bauhaus' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'bauhaus' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'bauhaus' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'bauhaus' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'bauhaus' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'bauhaus' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'bauhaus' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Bauhaus voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum 2d24',
			'Proceed to Checkout'                                                           => 'To the register 4f66',
			'Proceed to checkout'                                                           => 'To the register 2b93',
			'Lost your password?'                                                           => 'Key misplaced 4e6f',
			'Username or email address'                                                     => 'Bauhaus register 3033',
			'Username or Email Address'                                                     => 'Bauhaus register d04f',
			'+ Add apartment, suite, etc.'                                                  => 'Bauhaus register ded4',
			'You are currently checking out as a guest.'                                    => 'Bauhaus register ac59',
			'Showing the single result'                                                     => 'Bauhaus register b013',
			'Default sorting'                                                               => 'Counter choice ce71',
			'No products were found matching your selection.'                               => 'Shop-floor find 3d70',
			'No products in the cart.'                                                      => 'Parcel basket 2391',
			'Your cart is currently empty!'                                                 => 'Parcel basket c283',
			'Your cart is currently empty.'                                                 => 'Parcel basket 3686',
			'Return to shop'                                                                => 'Bauhaus register 9ce7',
			'Return To Shop'                                                                => 'Bauhaus register 389d',
			'Have a coupon?'                                                                => 'Voucher slip d88f',
			'Update cart'                                                                   => 'Parcel basket 85ba',
			'Place order'                                                                   => 'To the register 6527',
			'Apply coupon'                                                                  => 'Voucher slip 4200',
			'Coupon code'                                                                   => 'Voucher slip ac48',
			'Order details'                                                                 => 'Parcel record 5382',
			'Order summary'                                                                 => 'Parcel record 1bf8',
			'Cart subtotal'                                                                 => 'Parcel basket cf43',
			'Add to cart'                                                                   => 'Parcel basket be5c',
			'Customer details'                                                              => 'Bauhaus register 7950',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Bauhaus register df60',
			'Be the first to review'                                                        => 'Counter note 1ecc',
			'Your review'                                                                   => 'Counter note 3503',
			'Your rating'                                                                   => 'Bauhaus register a25c',
			'Submit'                                                                        => 'Bauhaus register 6086',
			'Description'                                                                   => 'Bauhaus register 4aba',
			'Reviews'                                                                       => 'Counter note 9b2e',
			'Additional information'                                                        => 'Bauhaus register 8bf3',
			'View cart'                                                                     => 'Parcel basket fa85',
			'View Cart'                                                                     => 'Parcel basket 635c',
			'Choose an option'                                                              => 'Bauhaus register 1b45',
			'Clear'                                                                         => 'Bauhaus register 604b',
			'Login'                                                                         => 'Bauhaus register 23f3',
			'Log in'                                                                        => 'Bauhaus register e7d7',
			'Log out'                                                                       => 'Bauhaus register 7c71',
			'Register'                                                                      => 'Bauhaus register 42e7',
			'Remember me'                                                                   => 'Bauhaus register 687c',
			'My account'                                                                    => 'Bauhaus register aeae',
			'My Account'                                                                    => 'Bauhaus register 34ed',
			'Order received'                                                                => 'Parcel record f325',
			'Thank you. Your order has been received.'                                      => 'Parcel record c122',
			'You may also like&hellip;'                                                     => 'Bauhaus register 52e3',
			'You may also like…'                                                            => 'Bauhaus register d0be',
			'Related products'                                                              => 'Shop-floor find 4012',
		);
...
// and callouts stay theme-distinct (Bauhaus = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'bauhaus_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'bauhaus_render_account_dashboard' ) ) {
	/**
	 * Render the Bauhaus-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Bauhaus voice.
	 ...
	 */
	function bauhaus_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'bauhaus' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'bauhaus' ),
				'title'   => __( 'Recent orders', 'bauhaus' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'bauhaus' ),
				'cta'     => __( 'View orders', 'bauhaus' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'bauhaus' ),
				'title'   => __( 'Saved addresses', 'bauhaus' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'bauhaus' ),
				'cta'     => __( 'Edit addresses', 'bauhaus' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'bauhaus' ),
				'title'   => __( 'Account details', 'bauhaus' ),
				'lede'    => __( 'Edit your name, email, and password.', 'bauhaus' ),
				'cta'     => __( 'Edit profile', 'bauhaus' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'bauhaus' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'bauhaus' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'bauhaus' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Bauhaus') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Bauhaus and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'bauhaus' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Bauhaus.', 'bauhaus' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'bauhaus' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'bauhaus' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'bauhaus' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'bauhaus' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'bauhaus' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'bauhaus' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `bauhaus_swatches_color_map()` so
...
if ( ! function_exists( 'bauhaus_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function bauhaus_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = bauhaus_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return bauhaus_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'bauhaus' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'bauhaus' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'bauhaus' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'bauhaus' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'bauhaus' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'bauhaus' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'bauhaus' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'bauhaus' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'bauhaus' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'bauhaus' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'bauhaus' );
			}
			$eyebrow = __( 'Shop', 'bauhaus' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'bauhaus' ) );
		?>
<script>
(function(){
