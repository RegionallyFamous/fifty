# Bauhaus — design brief

This file was emitted by `bin/design.py` after cloning `obel` -> `bauhaus` and applying the spec's palette + font choices. Read it before you do anything else; it captures the design intent the spec encoded so you can write the per-theme microcopy block, restyle the templates that need it, and brief any product photography in the same voice.

## Tagline

> circle + square + triangle hero icon, red + yellow + blue + cream, Universal-style geometric type

## Voice

pre-1950 stationery store: warm, direct, no exclamation marks.

Write this voice into the `// === BEGIN wc microcopy ===` block in `bauhaus/functions.php`. Every theme's microcopy must read distinctly from every other theme's; `bin/check.py check_wc_microcopy_distinct_across_themes` enforces it. Crib the structure from any sibling theme's microcopy block, then rewrite every literal string in this voice.
...
point at `bauhaus/assets/fonts/<slug>-<weight>.woff2` — drop the files there):

- `Universal` (display, weights 400, 700)
- `Futura Book` (sans, weights 400, 600)
...
2. Drop product photographs as `bauhaus/playground/images/product-wo-*.jpg` 
...
5. Re-shoot screenshot.png: `python3 bin/build-theme-screenshots.py bauhaus`.
6. Snap baseline: `python3 bin/snap.py shoot bauhaus && \
   python3 bin/snap.py baseline bauhaus`.
7. Verify: `python3 bin/check.py bauhaus --quick` — fix every failure before
...
_Brief auto-generated for bauhaus by `bin/design.py`._
