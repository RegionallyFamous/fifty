...
Obel is a block (FSE) theme for WordPress and WooCommerce stores. It ships full template coverage for both, composed entirely of core WordPress blocks and core WooCommerce blocks. All visual styling lives in theme.json. There is no custom CSS, no custom blocks, and no patterns library.
...
1. Upload the `obel` folder to `/wp-content/themes/`.
...
Patterns tend to be project-specific. Obel is a starter, so patterns belong in the per-project clone rather than in the framework.
...
Obel ships with zero custom CSS by design. If you must add CSS, use *Appearance > Editor > Styles > Additional CSS*, or use the `styles.blocks.<name>.css` escape hatch in `theme.json` (WordPress 6.6+).

== Copyright ==

Obel WordPress Theme, (C) 2026 Obel.
Obel is distributed under the terms of the GNU GPL v2 or later.
