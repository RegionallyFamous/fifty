...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'aero_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'aero_render_account_dashboard' ) ) {
	/**
	 * Render the Aero-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short spec-sheet lede in the Aero voice.
	 ...
	 */
	function aero_render_account_dashboard(): void {
		...
		$cards = array(
			array(
				'eyebrow' => __( 'Manifest', 'aero' ),
				'title'   => __( 'Recent runs', 'aero' ),
				'lede'    => __( 'Live shipping status and one-tap reorder for prior runs.', 'aero' ),
				'cta'     => __( 'Open manifest', 'aero' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Routing', 'aero' ),
				'title'   => __( 'Saved coordinates', 'aero' ),
				'lede'    => __( 'Adjust ship-to points and billing endpoints for faster checkout.', 'aero' ),
				'cta'     => __( 'Edit coordinates', 'aero' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Pilot ID', 'aero' ),
				'title'   => __( 'Credentials', 'aero' ),
				'lede'    => __( 'Update name, contact channel, and access password.', 'aero' ),
				'cta'     => __( 'Adjust credentials', 'aero' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Telemetry', 'aero' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Pilot %s.', 'aero' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Open the flight log, recall a manifest, or update saved coordinates.', 'aero' ); ?></p>
	</header>
...
// sport/technical split. Brand name is hardcoded ('Aero') so the
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Pilot ID', 'aero' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Aero.', 'aero' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to pull a manifest, watch a shipment in flight, and clear checkout faster.', 'aero' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'Repeat-order in one tap', 'aero' ); ?></li>
		<li><?php esc_html_e( 'Coordinates on standby', 'aero' ); ?></li>
		<li><?php esc_html_e( 'Priority boarding on new drops', 'aero' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		/* translators: %s: contact link wrapping the words "hail us". */
		esc_html__( "Can't sign in? %s.", 'aero' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Hail us', 'aero' ) . '</a>'
	);
?></p>
		<?php
	},
...
// generic WC `<select>` with iridescent glass pill swatches; the
// hidden select stays in the DOM as WC's source of truth and an
// inline JS shim forwards button clicks into its `change` event.
if ( ! function_exists( 'aero_swatches_color_map' ) ) {
	function aero_swatches_color_map(): array {
		return array(
			'amber'    => '#c98018',
			'clear'    => '#e8e3d8',
			'black'    => '#0a0a0a',
...
if ( ! function_exists( 'aero_swatches_render_group' ) ) {
	function aero_swatches_render_group( string $default_html, array $args ): string {
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return aero_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
// WC's default banners with a sport/technical eyebrow + display
// heading + 2-CTA strip.
add_action(
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Basket', 'aero' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Basket holds nothing yet.', 'aero' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the drop, or queue something up from the log.', 'aero' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Browse the drop', 'aero' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Open the flight log', 'aero' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Catalogue', 'aero' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'No items match those parameters.', 'aero' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Loosen the filters, or step through the whole catalogue.', 'aero' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Show every listing', 'aero' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Open the flight log', 'aero' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Catalogue', 'aero' );
			}
			$eyebrow = __( 'Catalogue', 'aero' );
		}
...
// === BEGIN payment-icons ===
//
// "Payments processed" trust strip on cart + checkout. Inline-SVG
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Payments processed', 'aero' ) );
		?>
<script>
(function(){
