...
1. Make the change.
2. `python3 bin/snap.py shoot <theme> --routes <route>` — leave the `--viewports` flag OFF so all four (`mobile`, `tablet`, `desktop`, `wide`) are captured. If you must scope it for speed, the floor is `--viewports mobile,desktop` — never just one viewport (see root-rule #18). The same applies to ad-hoc Playwright probes you write under `/tmp/`: iterate at minimum over `[("mobile", 375, 812), ("desktop", 1280, 800)]`.
