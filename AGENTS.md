...
16. **Mu-plugin overrides MUST use `render_block_<block-name>` filters, not `woocommerce_*_shop_loop` echoes.** WooCommerce's legacy `woocommerce_before_shop_loop`, `woocommerce_after_shop_loop`, and `woocommerce_no_products_found` actions fire in TWO places on every modern shop / archive page: (a) the legacy `woocommerce_content()` shortcode loop (where the original WC counter, sort dropdown, and "no products" notice all hooked in), AND (b) INSIDE `wp:woocommerce/product-collection`'s server render — the product-collection block invokes the loop hooks for backwards compatibility with sidebar widgets and addon plugins. Themes built on the modern block editor place `wp:woocommerce/product-results-count` (and `catalog-sorting`, etc.) inside a flex header row in `archive-product.html` and then render the product grid below via `wp:woocommerce/product-collection`. A mu-plugin that echoes HTML on `woocommerce_before_shop_loop` paints that HTML TWICE: once in the title row (block-rendered, correct position) AND once floating above the product grid with no parent flex container. The previous `playground/wo-microcopy-mu.php` shipped exactly this regression — a `<p class="woocommerce-result-count wo-result-count">N items</p>` rendered in both spots, the second copy appearing as a naked "23 ITEMS" line in the middle of nowhere on every archive page; the static gate missed it because every existing check inspected source files in isolation, never the runtime interaction between mu-plugin echoes and modern shop block composition. The canonical pattern is `render_block_<block-name>` filters: they receive the block's already-correctly-positioned HTML and rewrite it in place, so there is exactly ONE node per surface and it sits where the template author put it (see the comment block above the `render_block_woocommerce/product-results-count` filter in `playground/wo-microcopy-mu.php` for the full post-mortem and the working pattern). `bin/check.py`'s `check_mu_plugin_no_legacy_loop_echoes` enforces this by parsing every `add_action('woocommerce_before_shop_loop' | 'woocommerce_after_shop_loop' | 'woocommerce_no_products_found', function () { ... })` in the mu-plugin and failing if the callback body emits HTML (`echo '<...>'`, `print '<...>'`, `printf('<...>')`, or a `<<<EOT` heredoc with markup). NEVER add a new HTML-emitting echo on these hooks; if a future override needs to fire on a block surface, find its `render_block_<block-name>` filter and rewrite there instead.
