...
**Miles → theme factory (default design input):** Fifty does **not** call Claude/Anthropic to invent specs by default. **Supported paths:** (1) **Miles** — export validated **spec JSON** + **`miles-ready.json`** (`site_ready`, optional `spec` path); bridge with `python3 bin/miles-bridge-to-spec.py --slug <slug> --name "<Display Name>" --artifacts-dir <dir>`, then `python3 bin/design.py --spec …` or `python3 bin/design.py --miles-artifacts <dir> --miles-slug <slug> --miles-name "<Display Name>"` (runs **`miles whoami`** first unless **`FIFTY_SKIP_MILES_GATE=1`**). (2) **Concept queue** — `bin/concept-to-spec.py <slug>` uses **deterministic** seed→spec mapping only. (3) **Hand-authored** `tmp/specs/<slug>.json` + `--spec`. **Legacy escape hatch** (Anthropic `spec-from-prompt`, `design.py --prompt`, manifest `prompt` rows, `concept-to-spec --llm`): set **`FIFTY_ALLOW_NON_MILES_SPEC=1`** — not the default product path.

| Skill | Use when |
|---|---|
| `.claude/skills/design-theme/SKILL.md` | Shipping a brand-new theme from a **Miles export** or **concept seed** (deterministic `concept-to-spec`). Owns the spine: validated `spec.json` → `bin/design.py` (clone + token swap + seed + sync + check). Pair with `docs/shipping-a-theme.md`. |
...
- **Miles export or concept seed (no finished mockup workflow)** → `.claude/skills/design-theme/SKILL.md`. Drives `bin/design.py` end-to-end: **Miles** (`--miles-artifacts` / bridge) or **deterministic** `bin/concept-to-spec.py` → `spec.json` → clone → token swap → seed → sync → check.
...
| Script | Use when |
|---|---|
| `bin/concept-to-spec.py <slug>` | Generate a `bin/design.py` spec from a concept seed (**deterministic** default). Legacy vision polish: `--llm` only when `FIFTY_ALLOW_NON_MILES_SPEC=1`. Output: `tmp/specs/<slug>.json`. |
