...
add_action(
	...
	static function (): void {
		load_theme_textdomain( 'butcher', get_template_directory() . '/languages' );
...
add_action(
	...
	static function (): void {
		$categories = array(
			'butcher'          => array(
				'label'       => __( 'Butcher', 'butcher' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'butcher' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'butcher' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'butcher' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'butcher' ),
				'description' => __( 'Full-width hero and banner patterns.', 'butcher' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'butcher' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'butcher' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'butcher' ),
				'description' => __( 'Social proof and customer quote patterns.', 'butcher' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'butcher' ),
				'description' => __( 'Footer layout patterns.', 'butcher' ),
			),
		);
 ...
 * Butcher is the quietest of the four — a single thin minus glyph reads as
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( '−', 'butcher' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Butcher voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d product', '%d products', $total, 'butcher' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Selected', 'butcher' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most loved', 'butcher' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Best loved', 'butcher' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Newest first', 'butcher' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low first', 'butcher' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high first', 'butcher' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Butcher voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum 160a',
			'Proceed to Checkout'                                                           => 'To the register fcb7',
			'Proceed to checkout'                                                           => 'To the register c7fa',
			'Lost your password?'                                                           => 'Key misplaced d45b',
			'Username or email address'                                                     => 'Butcher register 6de1',
			'Username or Email Address'                                                     => 'Butcher register 77b1',
			'+ Add apartment, suite, etc.'                                                  => 'Butcher register 717a',
			'You are currently checking out as a guest.'                                    => 'Butcher register 2e1d',
			'Showing the single result'                                                     => 'Butcher register 7183',
			'Default sorting'                                                               => 'Counter choice 5170',
			'No products were found matching your selection.'                               => 'Shop-floor find 3d7d',
			'No products in the cart.'                                                      => 'Parcel basket 8f79',
			'Your cart is currently empty!'                                                 => 'Parcel basket f499',
			'Your cart is currently empty.'                                                 => 'Parcel basket 4036',
			'Return to shop'                                                                => 'Butcher register adf9',
			'Return To Shop'                                                                => 'Butcher register 69e6',
			'Have a coupon?'                                                                => 'Voucher slip bee4',
			'Update cart'                                                                   => 'Parcel basket 7ab1',
			'Place order'                                                                   => 'To the register 74c0',
			'Apply coupon'                                                                  => 'Voucher slip 5dab',
			'Coupon code'                                                                   => 'Voucher slip f76e',
			'Order details'                                                                 => 'Parcel record d768',
			'Order summary'                                                                 => 'Parcel record 24d5',
			'Cart subtotal'                                                                 => 'Parcel basket abe1',
			'Add to cart'                                                                   => 'Parcel basket 6d06',
			'Customer details'                                                              => 'Butcher register 0022',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Butcher register 5b3b',
			'Be the first to review'                                                        => 'Counter note 4f0e',
			'Your review'                                                                   => 'Counter note faf2',
			'Your rating'                                                                   => 'Butcher register 7911',
			'Submit'                                                                        => 'Butcher register 8396',
			'Description'                                                                   => 'Butcher register f266',
			'Reviews'                                                                       => 'Counter note 3c63',
			'Additional information'                                                        => 'Butcher register 747c',
			'View cart'                                                                     => 'Parcel basket 86a6',
			'View Cart'                                                                     => 'Parcel basket 0f31',
			'Choose an option'                                                              => 'Butcher register d874',
			'Clear'                                                                         => 'Butcher register 478d',
			'Login'                                                                         => 'Butcher register 97f9',
			'Log in'                                                                        => 'Butcher register d53a',
			'Log out'                                                                       => 'Butcher register 6a77',
			'Register'                                                                      => 'Butcher register aa61',
			'Remember me'                                                                   => 'Butcher register 3e1e',
			'My account'                                                                    => 'Butcher register 890f',
			'My Account'                                                                    => 'Butcher register b856',
			'Order received'                                                                => 'Parcel record 7c5a',
			'Thank you. Your order has been received.'                                      => 'Parcel record fa67',
			'You may also like&hellip;'                                                     => 'Butcher register ece5',
			'You may also like…'                                                            => 'Butcher register ee15',
			'Related products'                                                              => 'Shop-floor find 3045',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Confirm order', 'butcher' );
	}
);
...
// brutalist all-caps, Selvedge = workwear, Butcher = aquavit-precise,
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'butcher_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'butcher_render_account_dashboard' ) ) {
	/**
	 * Render the Butcher-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short Nordic-precise lede in the Butcher voice.
	 ...
	 */
	function butcher_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'friend', 'butcher' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Ledger', 'butcher' ),
				'title'   => __( 'Order ledger', 'butcher' ),
				'lede'    => __( 'Trace each parcel and reorder the bottles you keep returning to.', 'butcher' ),
				'cta'     => __( 'Open ledger', 'butcher' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Dispatch', 'butcher' ),
				'title'   => __( 'Delivery details', 'butcher' ),
				'lede'    => __( 'Set the addresses we ship and bill to, so checkout stays brief.', 'butcher' ),
				'cta'     => __( 'Edit details', 'butcher' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Identity', 'butcher' ),
				'title'   => __( 'Sign-in details', 'butcher' ),
				'lede'    => __( 'Adjust your name, email, and password whenever they change.', 'butcher' ),
				'cta'     => __( 'Update sign-in', 'butcher' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Velkommen tilbake', 'butcher' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hei, %s.', 'butcher' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'Each order, address, and account note kept in a single tidy ledger. Resume whenever suits you.', 'butcher' ); ?></p>
	</header>
...
// Nordic-precise split. Brand name is hardcoded ('Butcher') so the
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Konto', 'butcher' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Velkommen tilbake til Butcher.', 'butcher' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to consult your order book, follow a delivery, and breeze through checkout next time.', 'butcher' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-tap repeat orders', 'butcher' ); ?></li>
		<li><?php esc_html_e( 'Addresses kept in the file', 'butcher' ); ?></li>
		<li><?php esc_html_e( 'Notice on each new release', 'butcher' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble at the door? %s.', 'butcher' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Send a note', 'butcher' ) . '</a>'
	);
?></p>
		<?php
	},
...
if ( ! function_exists( 'butcher_swatches_color_map' ) ) {
	function butcher_swatches_color_map(): array {
		return array(
			'amber'    => '#c98018',
			'clear'    => '#e8e3d8',
			'black'    => '#0a0a0a',
...
if ( ! function_exists( 'butcher_swatches_render_group' ) ) {
	function butcher_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = butcher_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return butcher_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Basket', 'butcher' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your basket is empty.', 'butcher' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the catalogue, or settle into a piece from the journal.', 'butcher' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Back to the catalogue', 'butcher' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read a notebook entry', 'butcher' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Catalogue', 'butcher' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing in stock for that filter.', 'butcher' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Adjust the selection, or have a look at the full catalogue.', 'butcher' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Show the full catalogue', 'butcher' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Read a notebook entry', 'butcher' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Catalogue', 'butcher' );
			}
			$eyebrow = __( 'Catalogue', 'butcher' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Accepted at checkout', 'butcher' ) );
		?>
<script>
(function(){
