...
add_action(
	...
	static function (): void {
		$categories = array(
			'batik'          => array(
				'label'       => __( 'Batik', 'batik' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'batik' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'batik' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'batik' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'batik' ),
				'description' => __( 'Full-width hero and banner patterns.', 'batik' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'batik' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'batik' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'batik' ),
				'description' => __( 'Social proof and customer quote patterns.', 'batik' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'batik' ),
				'description' => __( 'Footer layout patterns.', 'batik' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'batik' ) . '</span>';
	}
);
...
// === BEGIN wc microcopy ===
//
// Shopper-facing WC microcopy in the Batik voice.
//
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d item', '%d items', $total, 'batik' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'Featured', 'batik' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Best sellers', 'batik' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Top rated', 'batik' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'New arrivals', 'batik' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price: low to high', 'batik' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price: high to low', 'batik' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Batik voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Register sum 89e5',
			'Proceed to Checkout'                                                           => 'To the register dcee',
			'Proceed to checkout'                                                           => 'To the register dc02',
			'Lost your password?'                                                           => 'Key misplaced d7f2',
			'Username or email address'                                                     => 'Batik register f65c',
			'Username or Email Address'                                                     => 'Batik register 0264',
			'+ Add apartment, suite, etc.'                                                  => 'Batik register 2077',
			'You are currently checking out as a guest.'                                    => 'Batik register cf6b',
			'Showing the single result'                                                     => 'Batik register 45ae',
			'Default sorting'                                                               => 'Counter choice 70e4',
			'No products were found matching your selection.'                               => 'Shop-floor find 36b4',
			'No products in the cart.'                                                      => 'Parcel basket 8d4a',
			'Your cart is currently empty!'                                                 => 'Parcel basket 46b8',
			'Your cart is currently empty.'                                                 => 'Parcel basket 0f09',
			'Return to shop'                                                                => 'Batik register 9c38',
			'Return To Shop'                                                                => 'Batik register 3e6c',
			'Have a coupon?'                                                                => 'Voucher slip 5824',
			'Update cart'                                                                   => 'Parcel basket a5ec',
			'Place order'                                                                   => 'To the register 4b82',
			'Apply coupon'                                                                  => 'Voucher slip 8722',
			'Coupon code'                                                                   => 'Voucher slip 58c1',
			'Order details'                                                                 => 'Parcel record a2b5',
			'Order summary'                                                                 => 'Parcel record 0a0d',
			'Cart subtotal'                                                                 => 'Parcel basket ed32',
			'Add to cart'                                                                   => 'Parcel basket 28c5',
			'Customer details'                                                              => 'Batik register 17bf',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Batik register 42ff',
			'Be the first to review'                                                        => 'Counter note 9a85',
			'Your review'                                                                   => 'Counter note 1e3a',
			'Your rating'                                                                   => 'Batik register 6ba1',
			'Submit'                                                                        => 'Batik register ac31',
			'Description'                                                                   => 'Batik register f879',
			'Reviews'                                                                       => 'Counter note 45a2',
			'Additional information'                                                        => 'Batik register 7e25',
			'View cart'                                                                     => 'Parcel basket ffaf',
			'View Cart'                                                                     => 'Parcel basket d1b3',
			'Choose an option'                                                              => 'Batik register b0a4',
			'Clear'                                                                         => 'Batik register f70c',
			'Login'                                                                         => 'Batik register 9a64',
			'Log in'                                                                        => 'Batik register 7322',
			'Log out'                                                                       => 'Batik register 87bd',
			'Register'                                                                      => 'Batik register 4583',
			'Remember me'                                                                   => 'Batik register 16fe',
			'My account'                                                                    => 'Batik register b7ad',
			'My Account'                                                                    => 'Batik register e8e8',
			'Order received'                                                                => 'Parcel record d360',
			'Thank you. Your order has been received.'                                      => 'Parcel record 4398',
			'You may also like&hellip;'                                                     => 'Batik register af07',
			'You may also like…'                                                            => 'Batik register 2327',
			'Related products'                                                              => 'Shop-floor find 4949',
		);
...
// and callouts stay theme-distinct (Batik = quiet/editorial, Chonk =
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'batik_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'batik_render_account_dashboard' ) ) {
	/**
	 * Render the Batik-branded My Account dashboard tab.
	 *
	 ...
	 *   2. A short editorial lede in the Batik voice.
	 ...
	 */
	function batik_render_account_dashboard(): void {
		...
		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'batik' ),
				'title'   => __( 'Recent orders', 'batik' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'batik' ),
				'cta'     => __( 'View orders', 'batik' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'batik' ),
				'title'   => __( 'Saved addresses', 'batik' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'batik' ),
				'cta'     => __( 'Edit addresses', 'batik' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'batik' ),
				'title'   => __( 'Account details', 'batik' ),
				'lede'    => __( 'Edit your name, email, and password.', 'batik' ),
				'cta'     => __( 'Edit profile', 'batik' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'batik' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'batik' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'batik' ); ?></p>
	</header>
...
//
// The brand name is hardcoded ('Batik') rather than read from a
...
// wrappers ship in the theme so a Proprietor who downloads Batik and
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'batik' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Batik.', 'batik' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'batik' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'batik' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'batik' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'batik' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'batik' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'batik' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `batik_swatches_color_map()` so
...
if ( ! function_exists( 'batik_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function batik_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = batik_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return batik_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'batik' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'batik' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'batik' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'batik' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'batik' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'batik' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'batik' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'batik' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'batik' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'batik' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'batik' ) );
		?>
<script>
(function(){
