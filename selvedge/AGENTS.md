...
> Selvedge is one theme inside the **Fifty monorepo**. The monorepo lives one directory up; its `AGENTS.md` (`../AGENTS.md`) and `README.md` (`../README.md`) describe the layout and the rules that apply to every theme. Read this file for Selvedge-specific rules. Tooling lives in `../bin/`.

Instructions for AI coding agents working on the Selvedge theme. Read this file in full before making any changes. Human-oriented docs live in `README.md` and the [project wiki](https://github.com/RegionallyFamous/Fifty/wiki).
...
| `python3 ../bin/clone.py NEW_NAME` | Clone Selvedge into a new theme folder, renaming all identifiers. |
...
Selvedge is a block-only WooCommerce starter theme for WordPress. It is intended to be copied (use `python3 ../bin/clone.py NEW_NAME`) and then customized by editing `theme.json` and adding project-specific patterns. The framework itself is deliberately small.
...
9. **No raw WooCommerce frontend CSS bleeds through.** Any WC block whose default markup is styled by WC's `assets/css/woocommerce.css` (the rounded "folder" product tabs being the canonical example) must be re-styled in `theme.json` via `styles.blocks.<wc-block>.css` using project tokens, with WC's defaults explicitly nullified (`content: none`, `display: none`, `box-shadow: none`, `border-radius: 0`, etc. as needed). **Every rule must be prefixed with `html body &`** — WC's selectors are `(0,4,3)` (e.g. `.woocommerce div.product .woocommerce-tabs ul.tabs li`); a bare `&` prefix is only `(0,4,2)` and loses on every shared property. The `html body` prefix bumps to `(0,4,4)`, mirroring WC's own `is-style-minimal` style. When you build a new variant, port the override verbatim — never just copy the spacing/typography. The check script (`check_wc_overrides_styled`) enforces both the kill-defaults and the specificity bump for the known surfaces, currently `woocommerce/product-details` (the tabs).
...
| Long-form docs for humans | The [project wiki](https://github.com/RegionallyFamous/Selvedge/wiki). Do not commit Markdown docs into the repo (other than `README.md`, `INDEX.md`, `SYSTEM-PROMPT.md`, `AGENTS.md`, `CHANGELOG.md`). |
...
4. Set `Categories: selvedge-store` for project-style patterns or another registered category.
...
### Cloning Selvedge for a new project
...
For deeper reference, point the user (or read directly via `WebFetch` if you can) at the [project wiki](https://github.com/RegionallyFamous/Selvedge/wiki):

| Topic | Wiki page |
|---|---|
| Project structure (every file, what it does) | [Project-Structure](https://github.com/RegionallyFamous/Selvedge/wiki/Project-Structure) |
| Design tokens (which slug to use when) | [Design-Tokens](https://github.com/RegionallyFamous/Selvedge/wiki/Design-Tokens) |
| Step-by-step recipes for common tasks | [Recipes](https://github.com/RegionallyFamous/Selvedge/wiki/Recipes) |
| Bad-code/good-code pairs | [Anti-Patterns](https://github.com/RegionallyFamous/Selvedge/wiki/Anti-Patterns) |
| Inventory of every block referenced | [Block-Reference](https://github.com/RegionallyFamous/Selvedge/wiki/Block-Reference) |
| WooCommerce template guide | [WooCommerce-Integration](https://github.com/RegionallyFamous/Selvedge/wiki/WooCommerce-Integration) |
| Architecture and philosophy | [Architecture](https://github.com/RegionallyFamous/Selvedge/wiki/Architecture) |
| Style variation guide | [Style-Variations](https://github.com/RegionallyFamous/Selvedge/wiki/Style-Variations) |
| Tooling reference | [Tooling](https://github.com/RegionallyFamous/Selvedge/wiki/Tooling) |
...
4. For task-specific guidance, browse the [project wiki](https://github.com/RegionallyFamous/Selvedge/wiki).
