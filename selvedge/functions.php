...
// workwear split. Brand name is hardcoded ('Selvedge') so the
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Membership', 'selvedge' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Selvedge.', 'selvedge' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to pull up past orders, follow a shipment, and skip retyping at checkout.', 'selvedge' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'Reorder in two taps', 'selvedge' ); ?></li>
		<li><?php esc_html_e( 'Addresses kept on file', 'selvedge' ); ?></li>
		<li><?php esc_html_e( 'Mailing list — drops first', 'selvedge' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Locked out? %s.', 'selvedge' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Drop us a line', 'selvedge' ) . '</a>'
	);
?></p>
		<?php
	},
...
if ( ! function_exists( 'selvedge_swatches_color_map' ) ) {
	function selvedge_swatches_color_map(): array {
		return array(
			'amber'    => '#c98018',
			'clear'    => '#e8e3d8',
			'black'    => '#0a0a0a',
...
if ( ! function_exists( 'selvedge_swatches_render_group' ) ) {
	function selvedge_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = selvedge_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return selvedge_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	...
	<p class="wo-empty__ctas">
		...
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Open the journal', 'selvedge' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Floor', 'selvedge' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing fits those filters.', 'selvedge' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Cut the filters wider, or browse the whole shop floor.', 'selvedge' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Browse the floor', 'selvedge' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Open the journal', 'selvedge' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Shop floor', 'selvedge' );
			}
			$eyebrow = __( 'Shop floor', 'selvedge' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'Pays welcome', 'selvedge' ) );
		?>
<script>
(function(){
